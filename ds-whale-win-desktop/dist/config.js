...
(function () {
  ...
  const LEVEL_MIN = 1;
  const LEVEL_MAX = 20;
  function numToScale(v) { return SCALE_MIN + (v - LEVEL_MIN) * (SCALE_MAX - SCALE_MIN) / (LEVEL_MAX - LEVEL_MIN); }
  function scaleToNum(s) { return Math.round(LEVEL_MIN + (s - SCALE_MIN) * (LEVEL_MAX - LEVEL_MIN) / (SCALE_MAX - SCALE_MIN)); }
  ...
  function currentModels() { return activeProvider === 'codex' ? config.codexModels : config.models; }
  function currentBaseUrl() { return activeProvider === 'codex' ? config.codexBaseUrl : config.baseUrl; }
  ...
  function debouncedSave() {
    ...
    saveTimer = setTimeout(function () {
      invoke('save_config', { cfg: config })
        .then(function (saved) { config = saved; })
        .catch(function (err) { console.error('保存配置失败', err); });
    }, 400);
  }
  ...
  function saveWidgetDebounced() {
    ...
    widgetSaveTimer = setTimeout(function () {
      invoke('save_widget_config', { widget: config.widget })
        .then(function (saved) { config.widget = saved; })
        .catch(function (err) { console.error('保存挂件配置失败', err); });
    }, 400);
  }
  ...
  function saveDialogueDebounced() {
    ...
    dialogueSaveTimer = setTimeout(function () {
      invoke('save_dialogue', { dialogue: config.dialogue })
        .then(function (saved) { config.dialogue = saved; })
        .catch(function (err) { console.error('保存台词失败', err); });
    }, 400);
  }
    ...
    invoke('get_balance')
      .then(function (payload) {
        if (payload && payload.ok) {
          availableBalanceEl.textContent = '¥ ' + Number(payload.totalBalance || 0).toFixed(2);
          todayUsageEl.textContent = '¥ ' + Number(payload.todayUsage || 0).toFixed(2);
        } else {
  ...
  function applyDialogueToUi(dlg) {
    if (!dlg) dlg = { lines: DEFAULT_LINES.slice(), mode: 'random', intervalMin: 5, jitter: 0 };
    config.dialogue = dlg;
    if (dialogueModeEl) dialogueModeEl.value = (dlg.mode === 'carousel' || dlg.mode === 'random') ? dlg.mode : 'random';
    if (dialogueIntervalEl) dialogueIntervalEl.value = String(dlg.intervalMin || 5);
    if (dialogueJitterEl) dialogueJitterEl.value = String(dlg.jitter || 0);
    if (dialogueJitterValEl) dialogueJitterValEl.textContent = (dlg.jitter || 0) + '%';
    renderDialogueList();
  }
  ...
  function renderModels() {
    ...
    document.querySelectorAll('.model-row').forEach(function (row) {
      ...
      row.querySelector('[data-field="contextWindow"]').value = (m && m.contextWindow) || '';
    });
  ...
  provClaudeEl.addEventListener('click', function () { switchProvider('claude'); });
  provCodexEl.addEventListener('click', function () { switchProvider('codex'); });
  ...
  apiKeyEl.addEventListener('input', function (e) { config.apiKey = e.target.value.trim(); debouncedSave(); });

  baseUrlEl.addEventListener('input', function (e) {
    const v = e.target.value.trim();
    if (activeProvider === 'codex') config.codexBaseUrl = v; else config.baseUrl = v;
    debouncedSave();
  });
  ...
  document.querySelectorAll('.model-row').forEach(function (row) {
    ...
    row.querySelector('[data-field="contextWindow"]').addEventListener('input', function (e) {
  ...
  widgetScaleEl.addEventListener('input', function (e) {
    const level = Math.max(LEVEL_MIN, Math.min(LEVEL_MAX, Math.round(Number(e.target.value) || LEVEL_MIN)));
  ...
  if (addLineEl) addLineEl.addEventListener('click', function () {
    if (!config || !config.dialogue) return;
    expandDialogue();
  ...
  if (resetLinesEl) resetLinesEl.addEventListener('click', function () {
    if (!config || !config.dialogue) return;
    expandDialogue();
  ...
  if (dialogueModeEl) dialogueModeEl.addEventListener('change', function (e) {
    if (!config || !config.dialogue) return;
    config.dialogue.mode = e.target.value;
    saveDialogueDebounced();
  });

  if (dialogueIntervalEl) dialogueIntervalEl.addEventListener('input', function (e) {
    if (!config || !config.dialogue) return;
  ...
  if (dialogueJitterEl) dialogueJitterEl.addEventListener('input', function (e) {
    if (!config || !config.dialogue) return;
  ...
  if (toggleDialogueEl) toggleDialogueEl.addEventListener('click', function () {
  ...
  if (addCustomSoundEl) addCustomSoundEl.addEventListener('click', function () {
    invoke('pick_audio_file')
      .then(function (path) {
        if (!path) return;
        config.widget.customSounds = config.widget.customSounds || [];
        if (config.widget.customSounds.indexOf(path) === -1) config.widget.customSounds.push(path);
        const name = path.split(/[\\/]/).pop() || path;
        const opt = document.createElement('option');
        opt.value = path;
        opt.textContent = name;
      ...
      .catch(function (err) { console.error('选择音效失败', err); });
  });

  if (globalColorEl) globalColorEl.addEventListener('input', function (e) {
  ...
  if (bubbleColorEl) bubbleColorEl.addEventListener('input', function (e) {
    const hue = Number(e.target.value) || 0;
    const color = hexFromHue(hue);
  ...
  if (resetColorEl) resetColorEl.addEventListener('click', function () {
  ...
  widgetVolEl.addEventListener('input', function (e) {
    const v = Math.round(Math.min(1, Math.max(0, Number(e.target.value))) * 100) / 100;
    config.widget.vol = v;
  ...
  autostartEl.addEventListener('change', function (e) {
    ...
    invoke('set_autostart', { enabled: enabled })
      .then(function () { config.autostart = enabled; })
      .catch(function (err) { e.target.checked = !enabled; console.error('设置开机自启失败', err); });
  });
  ...
  confirmYesEl.addEventListener('click', function () {
    hideModal();
    invoke('open_external', { url: 'https://github.com/xiaolinnnnnnn/DeepSeek-Balance-Whale-Widget-Desktop/tree/DeepSeek-Balance-Whale-Widget-Desktop' })
      .catch(function (err) { console.error('打开外部链接失败', err); });
  });

  if (tutorialEl) tutorialEl.addEventListener('click', function (e) {
    e.preventDefault();
    invoke('open_external', { url: 'https://github.com/xiaolinnnnnnn/DeepSeek-Balance-Whale-Widget-Desktop/blob/DeepSeek-Balance-Whale-Widget-Desktop/README.md' })
      .catch(function (err) { console.error('打开外部链接失败', err); });
  });
