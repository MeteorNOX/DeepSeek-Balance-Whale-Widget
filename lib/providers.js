...
export const SCHEMES = {
  ...
  newapi: {
     ...
     */
    async read({ get, userToken, userId }) {
      ...
      try {
        ...
        // Best-effort: the quota integers are kept un-converted.
      }
 ...
 *   fetch?, timeoutMs?, maxBytes?, userToken?, userId?, guard? }`. `type` is
 *   one of `SCHEMES`. `credential` is the already-resolved key text;
 *   empty/absent is reported as `no-credential` (a friendly fact, not a
 *   fault). `apiKey` is accepted as a backward-compatible alias for
 *   `credential`. `userToken`/`userId` are the already-resolved New API
 *   user-balance credentials (`/api/user/self`). `guard` is an optional
...
export async function readAccount(options = {}) {
  ...
  try {
    const read = await scheme.read({ origin, get, now, userToken, userId });
