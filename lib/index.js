...
// —— 「不落 token 账单」的调用：web 检索与标题生成 ——
// DSH 的 web_search 工具每一轮都会用本账号向 Anthropic 兼容端点发一次
// deepseek-v4-flash 请求（会话事件 web/deepseek-search-llm-request），但这次调用的
// token 用量不会写进 assistant/message 事件 —— 只数 usage 的账会漏掉长任务里最大的一笔。
// 单价按账号账本（小鲸鱼记账）反推校准：
//   2026-09-08：账本 ¥31.67 − 会话 usage ¥9.32 = ¥22.35 ÷ 811 轮 ≈ ¥0.0276/轮
//   2026-08-28：账本 ¥3.46 − 会话 usage ¥1.41 = ¥2.05 ÷ 79 轮 ≈ ¥0.0259/轮
// 取 ¥0.027/轮；标题生成约 400 输入 + ≤64 输出 token ≈ ¥0.0015/次。
// 可在 .dshw-size.json 里写 searchUnitCost 覆盖（单位：元/轮）。
const DEFAULT_SEARCH_UNIT_COST = 0.027
const TITLE_LLM_UNIT_COST = 0.0015
const SEARCH_UNIT_CONFIG_TTL_MS = 30000
...
var css = [
  ...
  // 悬停行时 DSH 会把时间换成操作按钮，胶囊保持可见（否则没法悬停看明细）。
  '.dshwv-cost{flex:none;font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary,#9aa0a6);border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,.12));background:var(--dsw-alias-interactive-bg-hover,rgba(0,0,0,.03));border-radius:9px;padding:0 5px;margin:0 2px 0 4px;white-space:nowrap;transition:border-color .12s ease,background .12s ease}',
  '.dshwv-cost:hover{border-color:var(--dsw-alias-border-l3,rgba(0,0,0,.28));background:var(--dsw-alias-interactive-bg-active,rgba(0,0,0,.07))}',
  // 悬停明细面板：挂 body、fixed 定位，避免被侧边栏滚动容器裁掉
  '.dshwv-tip{position:fixed;left:0;top:0;display:none;max-width:330px;background:rgba(28,30,34,.97);color:#f2f4f7;border:1px solid rgba(255,255,255,.16);border-radius:10px;padding:9px 11px;font-size:12px;line-height:17px;box-shadow:0 8px 26px rgba(0,0,0,.32);pointer-events:none;z-index:2147483000;white-space:normal;text-align:left}',
  '.dshwv-tip.dshwv-tip-open{display:block}',
  '.dshwv-tip-h{display:flex;align-items:baseline;gap:6px;margin-bottom:6px}',
  '.dshwv-tip-h b{font-size:15px;font-weight:800;color:#ffd479}',
  '.dshwv-tip-sec{margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,.14)}',
  '.dshwv-tip-row{display:flex;justify-content:space-between;gap:12px}',
  '.dshwv-tip-row>span{color:#c2c9d4}',
  '.dshwv-tip-row>b{font-weight:600;flex:none}',
  '.dshwv-tip-row.dshwv-tip-own>span{color:#e8ecf3;font-weight:600}',
  '.dshwv-tip-ind{padding-left:9px}',
  '.dshwv-tip-dim{color:#8b93a1}',
  '.dshwv-tip-kid>span:first-child{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:200px}',
  '.dshwv-tip-foot{margin-top:6px;padding-top:6px;border-top:1px solid rgba(255,255,255,.14);color:#8b93a1;font-size:11px;line-height:15px}'
].join('\\n')
...
var costBySession = {}
// web 检索单价（元/轮），服务端随 costs 一起下发，仅用于提示文案
var costSearchUnitValue = 0.027
function costSearchUnit() {
  return typeof costSearchUnitValue === 'number' ? costSearchUnitValue : 0.027
}
// —— 悬停明细面板 ——
function costEsc(s) {
...
function costModelText(models) {
  var names = models ? Object.keys(models) : []
  if (names.length === 0) return '未知'
  if (names.length === 1) return names[0]
  return names.slice(0, 2).join(' / ') + (names.length > 2 ? ' 等' : '')
}
function costTokensText(hit, miss, out) {
  return '缓存 ' + fmtComp(hit) + ' · 输入 ' + fmtComp(miss) + ' · 输出 ' + fmtComp(out)
}
function costTipRow(label, value, cls) {
  return '<div class="dshwv-tip-row' + (cls ? ' ' + cls : '') + '"><span>' + costEsc(label) + '</span><b>' + costEsc(value) + '</b></div>'
}
// 子代理标题：优先用官方 session.list 的会话标题，取不到就用会话 id 前缀
function costChildLabel(id) {
  var m = costTitleMap
  var t = m && typeof m[id] === 'string' ? m[id] : ''
  if (!t) return String(id || '').slice(0, 8)
  return t.length > 16 ? t.slice(0, 16) + '…' : t
}
// 明细内容（纯字符串拼装，便于离线自测）：本会话 / 子代理 / 每个子代理
function costPanelHtml(info, labelOf) {
  var unit = costSearchUnit()
  var h = []
  var own = info.ownDetail || null
  var sub = info.subDetail || null
  h.push('<div class="dshwv-tip-h"><span>会话消耗</span><b>' + costEsc(fmtCost(info.amount)) + '</b>' +
    (info.subCount > 0 ? '<span class="dshwv-tip-dim">含 ' + info.subCount + ' 个子代理</span>' : '') + '</div>')
  if (own) {
    h.push('<div class="dshwv-tip-sec">')
    h.push(costTipRow('本会话', fmtCost(own.amount), 'dshwv-tip-own'))
    if (own.search > 0) h.push(costTipRow('· web 检索 ' + own.search + ' 轮', fmtCost(own.search * unit), 'dshwv-tip-ind'))
    h.push(costTipRow('· 模型', costModelText(own.models), 'dshwv-tip-ind dshwv-tip-dim'))
    h.push(costTipRow('· ' + costTokensText(own.hit, own.miss, own.out), '', 'dshwv-tip-ind dshwv-tip-dim'))
    h.push('</div>')
  }
  if (info.subCount > 0 && sub) {
    h.push('<div class="dshwv-tip-sec">')
    h.push(costTipRow('子代理 ' + info.subCount + ' 个', fmtCost(sub.amount), 'dshwv-tip-own'))
    if (sub.search > 0) h.push(costTipRow('· web 检索 ' + sub.search + ' 轮', fmtCost(sub.search * unit), 'dshwv-tip-ind'))
    h.push(costTipRow('· ' + costTokensText(sub.hit, sub.miss, sub.out), '', 'dshwv-tip-ind dshwv-tip-dim'))
    var kids = info.children || []
    for (var i = 0; i < kids.length && i < 8; i++) {
      var k = kids[i]
      var extra = []
      if (k.search > 0) extra.push('检索 ' + k.search + ' 轮')
      if (k.subCount > 0) extra.push('含 ' + k.subCount + ' 个')
      h.push('<div class="dshwv-tip-row dshwv-tip-kid dshwv-tip-ind"><span>' + costEsc((i + 1) + '. ' + (labelOf ? labelOf(k.id) : k.id)) + '</span><b>' +
        costEsc(fmtCost(k.amount)) + (extra.length > 0 ? ' <span class="dshwv-tip-dim">' + costEsc(extra.join(' · ')) + '</span>' : '') + '</b></div>')
    }
    if (kids.length > 8) h.push(costTipRow('… 还有 ' + (kids.length - 8) + ' 个', '', 'dshwv-tip-ind dshwv-tip-dim'))
    h.push('</div>')
  }
  if (info.search > 0) {
    h.push('<div class="dshwv-tip-foot">web 检索按 ¥' + costEsc(unit) + '/轮 估算（该调用的 token 不写进会话记录）</div>')
  } else {
    h.push('<div class="dshwv-tip-foot">按官方 token 定价 × 事件时刻峰谷换算</div>')
  }
  return h.join('')
}
var costTipEl = null
var costTipBadge = null
var costTipInfo = null
var costTipDirty = false
function ensureCostTip() {
  if (costTipEl && costTipEl.isConnected) return costTipEl
  costTipEl = document.createElement('div')
  costTipEl.className = 'dshwv-tip'
  document.body.appendChild(costTipEl)
  return costTipEl
}
function hideCostTip() {
  if (costTipEl) costTipEl.classList.remove('dshwv-tip-open')
  costTipBadge = null
  costTipInfo = null
}
function renderCostTip() {
  if (!costTipEl || !costTipInfo || !costTipBadge || !costTipBadge.isConnected) { hideCostTip(); return }
  costTipEl.innerHTML = costPanelHtml(costTipInfo, costChildLabel)
  costTipEl.classList.add('dshwv-tip-open')
  var r = costTipBadge.getBoundingClientRect()
  var w = costTipEl.offsetWidth || 260
  var h = costTipEl.offsetHeight || 120
  var left = r.right + 10
  if (left + w > window.innerWidth - 8) left = Math.max(8, r.left - w - 10)
  var top = Math.max(8, Math.min(r.top, window.innerHeight - h - 8))
  costTipEl.style.left = Math.round(left) + 'px'
  costTipEl.style.top = Math.round(top) + 'px'
}
function showCostTip(badge, info) {
  ensureCostTip()
  costTipBadge = badge
  costTipInfo = info
  renderCostTip()
  // 子代理标题来自官方 session.list，拿到后补一次渲染（标题只加载一次）
  if (info.children && info.children.length > 0 && costTitleMap === null) {
    loadCostTitleMap().then(function () {
      if (costTipInfo === info) renderCostTip()
    })
  }
}
// 悬停/移开：捕获阶段委托，徽标是 React 反复重建的，逐个绑定会漏
document.addEventListener('mouseover', function (e) {
  try {
    var el = e.target
    var badge = el && el.closest ? el.closest('.dshwv-cost') : null
    if (badge) {
      if (badge !== costTipBadge && badge.__dshwvInfo) showCostTip(badge, badge.__dshwvInfo)
      return
    }
    if (costTipBadge) hideCostTip()
  } catch (err) {}
}, true)
window.addEventListener('scroll', hideCostTip, true)
window.addEventListener('wheel', hideCostTip, { passive: true, capture: true })
window.addEventListener('resize', hideCostTip)
    ...
    fetch(COSTS_URL, { cache: 'no-store' })
      ...
      .then(function (d) {
        if (d && d.ok && d.costs && typeof d.costs === 'object') {
          ...
          if (typeof d.searchUnit === 'number') costSearchUnitValue = d.searchUnit
          applySessionBadges()
...
function applySessionBadges() {
  ...
  costApplyBusy = true
  // 徽标被 React 重建后，旧的悬停面板要么跟着消失、要么重绘成新数据
  if (costTipBadge && !costTipBadge.isConnected) hideCostTip()
  var diagRows = 0
  var diagMapped = 0
  var diagBadges = 0
  var diagSample = []
  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      // 明细走自绘面板（见 costPanelHtml）：原生 title 会和面板叠在一起，这里不再设置
      if (badge.title) badge.title = ''
      badge.__dshwvInfo = info
      if (badge === costTipBadge) costTipDirty = true
    ...
    // 面板开着时数据刷新了（每轮结束/轮询）：就地重绘，保持悬停不中断
    if (costTipDirty) {
      costTipDirty = false
      if (costTipBadge && costTipBadge.__dshwvInfo) {
        costTipInfo = costTipBadge.__dshwvInfo
        renderCostTip()
      }
    }
    reportCostDiag({
...
function apply(ctx) {
    ...
    // —— 侧边栏每会话消耗（模型感知版）——
    // 计价口径与「每轮消耗」一致：按每事件真实模型（flash/pro 档）+ 事件时刻峰谷定价，
    // 再加上不落 token 账单的辅助调用（web 检索轮次、标题生成）。live 会话直接扫描
    // 内存事件表（零落盘）；冷会话经 sessionPersistence.inspect 全量扫描一次后缓存
    //（持久化日志只读，进程期内无需重扫）。
    ...
    // web 检索单价：可在配置文件里改，30 秒内缓存一次（扫描循环里热路径）
    let searchUnitCache = { at: 0, value: DEFAULT_SEARCH_UNIT_COST }
    function searchUnitCost() {
      const now = Date.now()
      if (now - searchUnitCache.at < SEARCH_UNIT_CONFIG_TTL_MS) return searchUnitCache.value
      const cfg = readSizeConfig()
      const v = cfg && typeof cfg.searchUnitCost === 'number' && cfg.searchUnitCost >= 0
        ? cfg.searchUnitCost
        : DEFAULT_SEARCH_UNIT_COST
      searchUnitCache = { at: now, value: v }
      return v
    }
    // 每会话消耗 = usage 计费（assistant/message 的 token）+ 辅助调用计费
    // （web 检索轮次、会话标题生成）。返回 amount 为两者之和，aux/search 供展示拆分。
    function scanSessionEvents(events, units) {
      let amount = 0
      let hit = 0
      let miss = 0
      let out = 0
      ...
      const u = units || { search: searchUnitCost(), title: TITLE_LLM_UNIT_COST }
      for (const ev of events) {
        if (ev && ev.type === 'web/deepseek-search-llm-request') { search++; continue }
        if (ev && ev.type === 'session/title-llm-request') { titleCalls++; continue }
      ...
      const usage = amount
      const aux = search * u.search + titleCalls * u.title
      return { amount: usage + aux, usage, aux, search, titleCalls, hit, miss, out, models }
    }
    ...
    async function computeSessionCosts(records) {
      ...
      const units = { search: searchUnitCost(), title: TITLE_LLM_UNIT_COST }
      for (const rec of records) {
        ...
        if (rec.live) {
          ...
          const s = scanSessionEvents(session.events, units)
    ...
    async function scanOneCold(id) {
      try {
        ...
        const s = scanSessionEvents(loaded.events, { search: searchUnitCost(), title: TITLE_LLM_UNIT_COST })
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              searchUnitCost: typeof parsed.searchUnitCost === 'number' && parsed.searchUnitCost >= 0
                ? parsed.searchUnitCost
                : DEFAULT_SEARCH_UNIT_COST,
            }
          }
        } catch (err) {}
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx) {
      ...
      // 保留手改过的 web 检索单价（设置面板不提供该项，不能因为改别的设置被抹掉）
      ...
      const body = JSON.stringify({
        ...
        scrollGapOn: sgo,
        scrollGapPx: sgp,
        ...(prevCfg && typeof prevCfg.searchUnitCost === 'number' ? { searchUnitCost: prevCfg.searchUnitCost } : {}),
        updatedAt: new Date().toISOString(),
      })
      for (const p of SIZE_FILE_CANDIDATES) {
        try {
    ...
    // 子代理聚合：把 origin=subagent 的子孙会话费用并入根会话。
    // records 含 header.parentSession，构建 parent→children 表，DFS 求和。
    // 返回 rootId -> { amount(整组), own(自身明细), sub/subCount(子孙金额与个数),
    //   usage/aux/search/titleCalls/hit/miss/out/models(整组合计),
    //   subDetail(整组 − 自身，供悬停明细拆分), children(每个直接子代理的整组) }
    function aggregateSessionCosts(records, ownCosts) {
      const children = new Map()
      const rootIds = []
      for (const rec of records) {
        const id = rec && rec.header ? rec.header.id : null
        if (!id) continue
        const parent = rec.header.parentSession
        if (parent && parent !== id) {
      ...
      // 会话自身的完整明细（含未落 token 账单的辅助调用）
      const detailOf = (c) => ({
        amount: c.amount,
        usage: c.usage !== undefined ? c.usage : c.amount,
        aux: c.aux || 0,
        search: c.search || 0,
        titleCalls: c.titleCalls || 0,
        hit: c.hit,
        miss: c.miss,
        out: c.out,
        models: { ...c.models },
      ...
      const merged = {}
      const visit = (id) => {
        const own = ownCosts[id]
        if (!own) return null
        const self = detailOf(own)
        let sub = 0
        let subCount = 0
        let usage = self.usage
        let aux = self.aux
        let search = self.search
        let titleCalls = self.titleCalls
        let hit = own.hit
        let miss = own.miss
        let out = own.out
        ...
        for (const cid of children.get(id) || []) {
          const c = visit(cid)
          if (!c) continue
          sub += c.amount // 该子节点整组总价（含孙辈）
          subCount += c.subCount + 1
          usage += c.usage
          aux += c.aux
          search += c.search
          titleCalls += c.titleCalls
          hit += c.hit
          miss += c.miss
          out += c.out
          for (const m in c.models) {
            models[m] = (models[m] || 0) + c.models[m]
            subModels[m] = (subModels[m] || 0) + c.models[m]
          }
          kids.push(c)
        }
        return {
          id,
          amount: self.amount + sub,
          own: self,
          sub,
          subCount,
          usage,
          ...
          subDetail: {
            ...
            titleCalls: titleCalls - self.titleCalls,
            hit: hit - self.hit,
            miss: miss - self.miss,
            out: out - self.out,
            models: subModels,
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const aggregated = aggregateSessionCosts(records, costs)
          const unitSearch = searchUnitCost()
          const payload = { ok: true, costs: {}, pending: result.pending, searchUnit: unitSearch }
          const r4 = (n) => Math.round((Number(n) || 0) * 10000) / 10000
          // 悬停明细用：把「自身 / 子代理」两段的金额、检索轮次、token 拆分都带上
          const packDetail = (d) => ({
            amount: r4(d.amount),
            usage: r4(d.usage),
            aux: r4(d.aux),
            search: d.search || 0,
            titleCalls: d.titleCalls || 0,
            hit: d.hit,
            miss: d.miss,
            out: d.out,
            models: d.models,
          ...
          for (const id of Object.keys(aggregated)) {
            ...
            payload.costs[id] = {
              ...
              out: c.out,
              models: c.models,
              ownDetail: packDetail(c.own),
              subDetail: packDetail(c.subDetail),
              // 每个直接子代理一整组（含它自己的孙辈）；前端最多展示前若干个
              children: c.children.slice(0, 12).map((k) => ({
                id: k.id,
                amount: r4(k.amount),
                own: r4(k.own.amount),
                sub: r4(k.sub),
    ...
    disposers.push(ctx.webServer.tapIndex((html) => {
      ...
      const tag = '<script defer src="/dsh-whale/widget.js?v=0.2.14"></script>'
