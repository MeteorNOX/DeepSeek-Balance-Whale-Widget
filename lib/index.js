...
var css = [
  '.dshwv-root{position:fixed;right:0;bottom:0;touch-action:none;--dshw-scale:1;--dshw-base:clamp(122px,calc(min(250px,min(100vw,100vh) * 0.28) * var(--dshw-scale)),625px);width:var(--dshw-base);height:var(--dshw-base);pointer-events:none;user-select:none;-webkit-user-select:none;z-index:9999;font-family:inherit;transition:left .16s ease,top .16s ease,transform .3s ease}',
  ...
  '.dshwv-menu-btn{position:absolute;top:calc(40.55% + 4px);right:4px;width:26px;height:26px;border:none;border-radius:6px;background:rgba(32,49,112,.85);cursor:pointer;pointer-events:none;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;padding:0;z-index:2;opacity:0;transition:opacity .15s ease}',
  '/* 触屏无 hover：菜单按钮常驻可见 */',
  '.dshwv-menu-btn.dshwv-menu-btn-visible{opacity:1;pointer-events:auto}',
...
menuBox.addEventListener('click', function (e) {
  // 触屏无 hover：点菜单空白处（非控件）= 收起菜单
  var t = e.target
  if (t && t.closest && t.closest('input, select, button, .dshwv-menu-row, label')) return
  closeMenu()
})
...
function artBox610() { return artBBox || { l: 0, r: 610, t: 0, b: 610 } }
function padL() { var b = artBox610(); var sc = (root.offsetWidth || 1) / 610; return (root.classList.contains('dshwv-left') || state.h === 'left' ? 610 - b.r : b.l) * sc }
function padR() { var b = artBox610(); var sc = (root.offsetWidth || 1) / 610; return (root.classList.contains('dshwv-left') || state.h === 'left' ? b.l : 610 - b.r) * sc }
function padT() { return artBox610().t * ((root.offsetHeight || 1) / 610) }
function padB() { return (610 - artBox610().b) * ((root.offsetHeight || 1) / 610) }
function artW() { var b = artBox610(); return (b.r - b.l) * ((root.offsetWidth || 1) / 610) }
function artH() { var b = artBox610(); return (b.b - b.t) * ((root.offsetHeight || 1) / 610) }
...
function express() {
  ...
  var __vp = viewport()
  var __w = root.offsetWidth || 1
  var __h = root.offsetHeight || 1
  var __b = artBox610()
  var __sx = __w / 610
  var __sy = __h / 610
  var __padl = (root.classList.contains('dshwv-left') || state.h === 'left') ? (610 - __b.r) * __sx : __b.l * __sx
  var __padt = __b.t * __sy
  var __aw = (__b.r - __b.l) * __sx
  var __ah = (__b.b - __b.t) * __sy
  var __ax = Math.max(0, Math.min(state.left, __vp.w - __aw))
  var __ay = Math.max(0, Math.min(state.top, __vp.h - __ah))
  root.style.left = (__ax - __padl) + 'px'
  root.style.top = (__ay - __padt) + 'px'
...
function settle() {
  ...
  if (drag && drag.active) {
    // mid-drag resize: keep the pointer-follow position, clamp into the art-visible range
    state.left = clamp(state.left, 0, Math.max(0, vp.w - artW()))
    state.top = clamp(state.top, 0, Math.max(0, vp.h - artH()))
    express()
    return
  }
  if (state.h === 'right') {
    state.left = Math.max(0, vp.w - artW())
  } else if (state.h === 'left') {
    state.left = 0
  } else {
    state.left = clamp(state.left, 0, Math.max(0, vp.w - artW()))
  }  if (state.v === 'bottom') {
    state.top = Math.max(0, vp.h - artH())
  } else if (state.v === 'top') {
    state.top = 0
  } else {
    state.top = clamp(state.top, 0, Math.max(0, vp.h - artH()))
...
function toggleMenu() {
  ...
  else menuBtn.classList.remove('dshwv-menu-btn-visible')
}
function closeMenu() {
  menuOpen = false
...
function positionMenu() {
  try {
    ...
    // 顶部空间不足时翻到按钮下方，避免菜单伸出屏幕外只剩白边
    var mh = menuBox.offsetHeight
    if (b.top - mh < 8 && b.bottom + mh <= vp.h - 8) {
      menuBox.style.bottom = 'auto'
      menuBox.style.top = (b.bottom + 4) + 'px'
  ...
  try {
    ...
    probe.onload = function () {
      try {
        ...
        try {
          var dpx = hitCanvas.getContext('2d').getImageData(0, 0, 610, 610).data
          var abl = 610, abr = 0, abt = 610, abb = 0
          for (var ay = 0; ay < 610; ay += 2) for (var ax = 0; ax < 610; ax += 2) {
            var av = dpx[(ay * 610 + ax) * 4 + 3]
            if (av > 10) { if (ax < abl) abl = ax; if (ax > abr) abr = ax; if (ay < abt) abt = ay; if (ay > abb) abb = ay }
          }
          if (abl <= abr && abt <= abb) artBBox = { l: abl, r: abr, t: abt, b: abb }
        } catch (err) {}
        hitReady = true
      } catch (err) {}
    }
...
function onDocPointerDown(e) {
  ...
  if (menuOpen) {
    closeMenu()
    // 本次 tap 用于收菜单：吞掉后续 click，防止穿透误触下方元素
    window.dshwMenuJustClosed = true
    setTimeout(function () { window.dshwMenuJustClosed = false }, 700)
    return
  }
  ...
  var vp = viewport()
  var rect = root.getBoundingClientRect()
  drag = { active: true, startX: e.clientX, startY: e.clientY, origLeft: rect.left + padL(), origTop: rect.top + padT(), w: rect.width, h: rect.height, moved: false, lpPath: 0, vp: vp }
  drag.srcEvent = e
  // 长按 550ms 开菜单（移动端惯例；期间移动超阈值或抬手则取消，与拖拽互斥）
  clearTimeout(window.dshwLpTimer)
  window.dshwLpTimer = setTimeout(function () {
    if (drag && drag.active && !drag.moved && (drag.lpPath || 0) <= 20) {
      drag.cancelled = true
      try { if (navigator.vibrate) navigator.vibrate(30) } catch (err) {}
      endDrag(drag.srcEvent || e, false)
      toggleMenu()
    }
  }, 550)
...
function onDocPointerMove(e) {
  ...
  drag.lpPath += Math.abs(e.clientX - (drag.lastX === undefined ? drag.startX : drag.lastX)) + Math.abs(e.clientY - (drag.lastY === undefined ? drag.startY : drag.lastY))
  drag.lastX = e.clientX
  drag.lastY = e.clientY
  if (drag.lpPath > 20) clearTimeout(window.dshwLpTimer)
...
function onDocPointerCancel(e) { // 手势被浏览器接管/系统侧边手势中断：按拖拽方向顺势完成贴边（横向/纵向独立判断），无明确方向则保持原地
  if (drag && drag.active) {
    drag.cancelled = true
    var dirX = state.left - drag.origLeft
    var dirY = state.top - drag.origTop
    var vw = viewport().w, vh = viewport().h
    var w = root.offsetWidth || 0, h = root.offsetHeight || 0
    if (dirX < -8) { state.h = 'left'; state.hOff = 0; state.left = 0 }
    else if (dirX > 8) { state.h = 'right'; state.hOff = 0; state.left = vw - artW() }
    if (dirY < -8) { state.v = 'top'; state.vOff = 0; state.top = 0 }
    else if (dirY > 8) { state.v = 'bottom'; state.vOff = 0; state.top = vh - artH() }
    settle(); saveConfig()
  }
  endDrag(e, false)
}
function onDocClickStopper(e) {
  ...
  // 菜单开启期 / 目标在挂件 UI 内：放行按钮与控件；刚收完菜单的 tap 吞掉，绝不透传
  if (menuOpen) return
  if (e.target && e.target.closest && (e.target.closest('.dshwv-menu') || e.target.closest('.dshwv-menu-btn') || e.target.closest('.dshwv-bubble'))) return
  if (window.dshwMenuJustClosed) { window.dshwMenuJustClosed = false; try { e.preventDefault(); e.stopPropagation() } catch (err) {} ; return }
  if (!isWhaleHit(e)) return
  try { e.preventDefault(); e.stopPropagation() } catch (err) {}
}
// 触屏适配：root 为 pointer-events:none 穿透设计，touch-action 链不含 root，
// 浏览器会把按住拖动判定为页面手势并发 pointercancel——用非被动 touch 监听 preventDefault 掐断手势判定
document.addEventListener('touchstart', function (e) {
  if (menuOpen) return
  if (e.touches && e.touches.length === 1) {
    var t = e.touches[0]
    if (isWhaleHit({ clientX: t.clientX, clientY: t.clientY })) { try { e.preventDefault() } catch (err) {} }
  }
}, { passive: false })
document.addEventListener('touchmove', function (e) {
  if (drag && drag.active) { try { e.preventDefault() } catch (err) {} }
}, { passive: false })
document.addEventListener('pointerdown', onDocPointerDown, true)
document.addEventListener('click', onDocClickStopper, true)
document.addEventListener('contextmenu', function (e) {
  // 长按鲸鱼不弹系统菜单/文本选择（菜单开启时全屏抑制）
  if (menuOpen || isWhaleHit(e)) { try { e.preventDefault() } catch (err) {} }
}, true)
...
function onDocPointerMoveCursor(e) {
  ...
  if (el && el.closest && (el.closest('.dshwv-bubble') || el.closest('.dshwv-menu') || el.closest('.dshwv-menu-btn'))) {
    setWidgetCursor('')
    if (e.pointerType === 'mouse') menuBtn.classList.add('dshwv-menu-btn-visible')
    return
  }
  var over = isWhaleHit(e)
  setWidgetCursor(over ? 'grab' : '')
  if (e.pointerType === 'mouse') menuBtn.classList.toggle('dshwv-menu-btn-visible', over || menuOpen)
}
...
function endDrag(e, clickAllowed) {
  ...
  // 取消型结束：回原位，不归边不 saveConfig
  if (drag.cancelled) { settle(); saveConfig(); drag.cancelled = false; return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
  var left = clamp(drag.origLeft + dx, 0, Math.max(0, drag.vp.w - artW()))
  var top = clamp(drag.origTop + dy, 0, Math.max(0, drag.vp.h - artH()))
  var centerX = left + artW() / 2
  if (centerX < drag.vp.w / 2) {
    state.h = 'left'
    state.hOff = 0
    left = 0
  } else {
...
function applyAnchorPos() {
  try {
    ...
    // 损坏特征同上
    if (a && a.v === 2 && a.hAnchor === 'left' && a.vAnchor === 'top' && a.hDist === 0 && a.vDist === 0) a = null
    ...
    state.left = clamp(l, 0, Math.max(0, vp.w - artW()))
    state.top = clamp(t, 0, Math.max(0, vp.h - artH()))
    state.h = a.hAnchor
    state.hOff = 0
    state.v = a.vAnchor
    state.vOff = 0
