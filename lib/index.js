  ...
  try {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      badge.title = '本会话消耗 ' + fmtCost(info.amount) + ' · ' + fmtModels(info.models) +
        ...
        (info.sub > 0 ? ' · 含子代理 ' + fmtCost(info.sub) + '（' + info.subCount + ' 个）' : '') +
        (info.search > 0
          ? ' · 其中 web 检索 ' + info.search + ' 轮 ' + fmtCost(info.search * costSearchUnit()) +
            '（DSH 不把检索请求的 token 写进会话事件，按 ¥' + costSearchUnit() + '/轮 估算）'
          : '')
...
function apply(ctx) {
    ...
    // 返回 rootId -> { amount(组), own(自身), sub(子孙), subCount, hit, miss, out, models }
    function aggregateSessionCosts(records, ownCosts) {
      ...
      const visit = (id) => {
        ...
        let sub = 0
        let subCount = 0
        let usage = own.usage !== undefined ? own.usage : own.amount
        let aux = own.aux || 0
        let search = own.search || 0
        let titleCalls = own.titleCalls || 0
        let hit = own.hit
        let miss = own.miss
        let out = own.out
        ...
        for (const cid of kids) {
          ...
          miss += c.miss
          out += c.out
          for (const m in c.models) models[m] = (models[m] || 0) + c.models[m]
        }
        return { amount: own.amount + sub, usage, aux, search, titleCalls, sub, subCount, hit, miss, out, models }
      }
        ...
        try {
          ...
          for (const id of Object.keys(aggregated)) {
            ...
            payload.costs[id] = {
              ...
              own: Math.round((c.amount - c.sub) * 10000) / 10000,
              sub: Math.round(c.sub * 10000) / 10000,
              subCount: c.subCount,
              usage: Math.round((c.usage || 0) * 10000) / 10000,
              aux: Math.round((c.aux || 0) * 10000) / 10000,
              search: c.search || 0,
    ...
    disposers.push(ctx.webServer.tapIndex((html) => {
      ...
      const tag = '<script defer src="/dsh-whale/widget.js?v=0.2.13"></script>'
