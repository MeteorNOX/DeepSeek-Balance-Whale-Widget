...
// flash 系列 2026-09-10 12:00（北京时间）起调价：缓存命中 0.05→0.02、未命中 1.5→1、输出 4.5→4（空闲价，
// 高峰价仍为空闲价的 2 倍）。生效时刻之前的历史分桶仍按旧价，所以按分桶时间选表。
const FLASH_PRICE_CUTOVER_SEC = Math.floor(Date.UTC(2026, 8, 10, 4, 0, 0) / 1000) // = 北京时间 2026-09-10 12:00
const FLASH_PRICE_OLD = { hit: [0.05, 0.1], miss: [1.5, 3.0], out: [4.5, 9.0] }
const BASE_PRICE = { hit: [0.02, 0.04], miss: [1, 2], out: [4, 8] }
// deepseek-v4-pro 为 flash 的 3 倍价（官方 2026-08-17 生效，本次未调价）；vision-exp 与 flash 同价
...
// timeSec = 该笔用量所属分桶的时间（epoch 秒）；缺省按当前时刻。
function priceFor(model, timeSec) {
  ...
  for (const key of Object.keys(PRICING)) {
    ...
    if (m.indexOf(key) !== -1) {
      ...
      // flash 系（含默认档）按分桶时间在调价前后切换；v4-pro 不受本次调价影响。
      return price === BASE_PRICE && t < FLASH_PRICE_CUTOVER_SEC ? FLASH_PRICE_OLD : price
    }
  }
  return t < FLASH_PRICE_CUTOVER_SEC ? FLASH_PRICE_OLD : BASE_PRICE
}
...
function apply(ctx) {
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        const off = isPeakTime(nowSec) ? 1 : 0
    ...
    function computeTodayUsage(data) {
      ...
      for (const s of series) {
        ...
        for (const b of buckets) {
          ...
          const p = priceFor(s.model, b.time)
