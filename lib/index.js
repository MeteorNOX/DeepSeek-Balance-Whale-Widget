...
import { decodePngToRgba, dominantHsl as pickDominantHsl } from './png.mjs'
...
export default {
  ...
  apply(ctx) {
    ...
    let balanceInFlightAccount = '' // 进行中的取余额请求属于哪个账号（keyRef）
    let gifBytes = null
    ...
    // —— DeepSeek 多账号（v0.3.3）——
    // 内置鲸鱼原本写死一个凭据名（DEEPSEEK_API_KEY），别人给的 key 没有位置放。
    // 这里把「用哪个凭据」变成可选账号：账号表存在注册表文件里，密钥仍然只进 DSH 凭据服务。
    // 读函数就地读注册表（readApiRegistry 是同作用域函数声明，会提升），写入统一走 apiSaveAccount。
    const DS_ACCOUNT_ID = /^[a-z0-9_-]{1,64}$/
    function apiRegistryOf() {
    ...
    // 账号 id 由凭据名派生：凭据名本来就在 POSIX 标识符字符集内，转小写即合法且稳定。
    // 稳定 id 让账本口径（observeBalance 的 scope）在切换账号后仍然各记各的。
    function deepseekAccountId(keyRef) {
      const id = String(keyRef || '').trim().toLowerCase()
      return DS_ACCOUNT_ID.test(id) ? id : ''
    }
    // 旧装（没有 accounts 段）等价于「只有一个 DEEPSEEK_API_KEY 账号」
    function readDeepseekAccounts() {
      const reg = apiRegistryOf()
      const raw = reg && Array.isArray(reg.accounts) ? reg.accounts : []
      const out = []
      const seen = new Set()
      for (const a of raw) {
        if (!a || typeof a !== 'object') continue
        const keyRef = String(a.keyRef || '').trim().slice(0, 64)
        const id = String(a.id || '').trim() || deepseekAccountId(keyRef)
        if (!id || !DS_ACCOUNT_ID.test(id) || seen.has(id)) continue
        seen.add(id)
        out.push({ id, name: String(a.name || '').trim().slice(0, 30) || id, keyRef })
      }
      if (!out.length) {
        return [{ id: 'deepseek_api_key', name: '默认账号', keyRef: 'DEEPSEEK_API_KEY', legacy: true }]
      }
      return out
    }
    function activeDeepseekAccount() {
      const accounts = readDeepseekAccounts()
      const reg = apiRegistryOf()
      const want = String((reg && reg.activeAccount) || '').trim()
      return accounts.find((a) => a.id === want) || accounts[0]
    }
    // 内置 DeepSeek 实际读取的凭据名：选中账号的凭据名 → 账号名本身（老配置把凭据名写在 name）→ DEEPSEEK_API_KEY
    function pickDeepseekKeyRef() {
      const a = activeDeepseekAccount() || {}
      for (const cand of [a.keyRef, a.name, 'DEEPSEEK_API_KEY']) {
        const s = String(cand || '').trim()
        if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(s)) return s
      }
      return 'DEEPSEEK_API_KEY'
    }
    // 解析「这次请求该用哪把 key」。账户快照与凭据一起返回：调用方必须用返回值里的 account
    // 记账/显示，不能在 await 之后再读一次 activeDeepseekAccount()——
    // 否则切换账号与刷新并发时，会用 A 账号的账户标签配上 B 账号的余额。
    async function resolveDeepseekCred() {
      // 这条防呆是整个多账号功能的成败点：**绝不能拿 A 的 key 冒充 B 的余额**。
      // 写成显式白名单而不是"顺手兜底"：只有当前账号自己声明的凭据名（以及它等于
      // DEEPSEEK_API_KEY 时的旧写法）才会去读，其它一律不猜。
      const account = activeDeepseekAccount() || {}
      const primary = pickDeepseekKeyRef()
      // 允许读取的凭据名：去重后按顺序试。
      const refs = []
      const pushRef = (r) => {
        const s = String(r || '').trim()
        if (s && refs.indexOf(s) === -1) refs.push(s)
      }
      pushRef(primary)
      // 仅当本账号声明的凭据名本身就是 DEEPSEEK_API_KEY 时，才认为"读 DEEPSEEK_API_KEY"是它的意图。
      // 选中别的账号时（primary 是别的名字），这里什么也不补 → 漏配就报"未配置 xxx"，不会回落。
      if (primary === 'DEEPSEEK_API_KEY') pushRef('DEEPSEEK_API_KEY')
      let picked = null
      for (const ref of refs) {
        let cred = null
        try { cred = await ctx.credentials.resolve(ref) } catch (err) { cred = null }
        if (cred && cred.value) { picked = { ref, cred }; break }
      }
      if (!picked) return { ref: primary, account, cred: null }
      return { ref: picked.ref, account, cred: picked.cred }
    }

    async function fetchBalance() {
      // 合并上游 v0.3.5 的冷启动重试与本分支的多账号解析：
      // v739（用户反馈「每次新实例的第一次余额请求都失败」）：
      // ① 冷启动时凭据服务可能尚未就绪（resolve 抛错），这与「确实没配置」不同 → 短延迟重试一次；
      // ② 实际用哪把 key 由 resolveDeepseekCred() 按当前选中账号决定（多账号）；
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        try {
          const hit = await resolveDeepseekCred()
          cred = hit.cred
          pickedRef = hit.ref
          account = hit.account || {}
          break
      ...
      if (!cred) {
        // account/accountKeyRef 一起带回：调用方用它显示"当前是谁"，
        // 且失败也要能对上账号（不会把上一次成功的余额贴到新账号上）。
        return {
          ok: false, code: 'NO_KEY', error: '未配置 ' + (pickedRef || 'DEEPSEEK_API_KEY'),
          account, accountKeyRef: String(pickedRef || ''),
        }
      }
      let lastErr = null
      for (let attempt = 0; attempt < 2; attempt++) {
        ...
        return {
          ...
          accountKeyRef: String(pickedRef || ''),
    ...
    // v0.3.3：keyRef 跟随「当前选中的账号」，前端也据此显示账号名。
    function apiBuiltinModel() {
      const t = API_TEMPLATES.deepseek
      const acct = activeDeepseekAccount() || {}
      return {
        id: API_BUILTIN_ID, name: t.name, provider: 'deepseek', currency: t.currency,
        keyRef: pickDeepseekKeyRef(), builtin: true, matchIds: ['deepseek'],
        accountId: String(acct.id || ''), accountName: String(acct.name || ''),
      }
    }
    ...
    // 只存「账号 → 凭据名」的映射与当前选中项；密钥本体永远只进 DSH 凭据服务。
    function commitApiRegistry(reg) {
      return writeApiRegistry(reg) ? { ok: true } : { ok: false, error: '注册表写入失败（检查 DSH 数据目录权限）' }
    }
    function setActiveDeepseekAccount(id) {
      const reg = readApiRegistry()
      const accounts = readDeepseekAccounts()
      const want = String(id || '').trim()
      const hit = accounts.find((a) => a.id === want)
      if (!hit) return { ok: false, error: '账号不存在：' + want }
      // 首次落盘时把「隐式默认账号」实体化，否则注册表里没有账号可选
      const explicit = Array.isArray(reg.accounts) && reg.accounts.length ? reg.accounts : accounts
      reg.accounts = explicit.map((a) => ({ id: a.id, name: a.name, keyRef: a.keyRef }))
      reg.activeAccount = hit.id
      const w = commitApiRegistry(reg)
      if (!w.ok) return w
      balanceCache = null
      balanceInFlight = null
      return { ok: true, accountId: hit.id, accountName: hit.name, keyRef: hit.keyRef }
    }
    async function apiSaveAccount(input) {
      const p = input || {}
      const name = String(p.name || '').trim().slice(0, 30)
      const keyRef = String(p.keyRef || '').trim().slice(0, 64)
      if (!name) return { ok: false, error: '请填写账号名称（仅用于你自己分辨，例如「同事A」）' }
      if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(keyRef)) {
        return { ok: false, error: '凭据名只能是字母/数字/下划线且不能以数字开头，例如 DEEPSEEK_KEY_B' }
      }
      const reg = readApiRegistry()
      const accounts = readDeepseekAccounts().map((a) => ({ id: a.id, name: a.name, keyRef: a.keyRef }))
      const byId = String(p.id || '').trim()
      let savedId = ''
      if (byId) {
        const hit = accounts.find((a) => a.id === byId)
        if (!hit) return { ok: false, error: '账号不存在：' + byId }
        hit.name = name
        hit.keyRef = keyRef
        savedId = hit.id
      } else {
        const id = deepseekAccountId(keyRef)
        if (!id) return { ok: false, error: '凭据名无法生成账号标识，请换一个名字' }
        if (accounts.some((a) => a.id === id)) return { ok: false, error: '该凭据名已存在：' + keyRef }
        accounts.push({ id, name, keyRef })
        savedId = id
      }
      reg.accounts = accounts
      if (!reg.activeAccount || !accounts.some((a) => a.id === reg.activeAccount)) reg.activeAccount = accounts[0].id
      const w = commitApiRegistry(reg)
      if (!w.ok) return w
      let keySaved = false
      if (p.keyValue !== undefined && String(p.keyValue).length) {
        try {
          ...
          return { ok: false, error: '账号已保存，但密钥写入凭据服务失败：' + String((err && err.message) || err).slice(0, 140) }
        }
      }
      balanceCache = null
      balanceInFlight = null
      return { ok: true, id: savedId, keySaved }
    }
    function apiDeleteAccount(input) {
      const id = String((input && input.id) || '').trim()
      const accounts = readDeepseekAccounts()
      if (accounts.length <= 1) return { ok: false, error: '至少保留一个 DeepSeek 账号' }
      const hit = accounts.find((a) => a.id === id)
      if (!hit) return { ok: false, error: '账号不存在：' + id }
      const rest = accounts.filter((a) => a.id !== id).map((a) => ({ id: a.id, name: a.name, keyRef: a.keyRef }))
      const reg = readApiRegistry()
      reg.accounts = rest
      if (String(reg.activeAccount || '') === id) reg.activeAccount = rest[0].id
      const w = commitApiRegistry(reg)
      if (!w.ok) return w
      balanceCache = null
      balanceInFlight = null
      // 凭据本体不在这里删：同一个凭据名可能被别处引用，删错了用户要重新配 key。
      // 想清掉密钥用「凭据管理」界面 / 面板里的「删密钥」。
      return { ok: true, removed: hit.id, keyRef: hit.keyRef, keyKept: true }
    }
    ...
    async function apiModelsPayload() {
      ...
      for (const m of models) {
        ...
        if (m.id === API_BUILTIN_ID) {
          ...
          // v0.3.3：账号列表 + 当前账号（前端「DeepSeek 账号」行据此渲染切换器）
          const dsAccounts = readDeepseekAccounts()
          const dsActive = activeDeepseekAccount() || {}
          entry.accountId = String(dsActive.id || '')
          entry.accountName = String(dsActive.name || '')
          entry.accounts = dsAccounts.map((a) => ({
            id: a.id, name: a.name, keyRef: a.keyRef,
            active: a.id === dsActive.id,
            hasKey: !!a.hasKey,
          }))
          let hitAccount = null
          try {
            // 一次解析出「账号 + 凭据」，随后所有显示与记账都用这个快照
            const hit = await resolveDeepseekCred()
            entry.hasKey = !!(hit.cred && hit.cred.value)
            entry.accountKeyRef = String(hit.ref || '')
            hitAccount = hit.account || null
            if (hitAccount) {
              entry.accountId = String(hitAccount.id || dsActive.id || '')
              entry.accountName = String(hitAccount.name || dsActive.name || '')
            }
          } catch (err) {}
          try {
            const p = await getBalance(false, hitAccount)
            if (p && p.ok) {
      ...
      return {
        ...
        // v0.3.3：DeepSeek 账号列表（供「DeepSeek 账号」切换器直接使用）
        accounts: readDeepseekAccounts().map((a) => ({
          id: a.id, name: a.name, keyRef: a.keyRef,
          active: a.id === (activeDeepseekAccount() || {}).id,
        })),
    ...
    // resolvedAccount：调用方（模型面板）在同一个请求里已经解析过的账号快照。
    // 传进来时以它为准，避免「解析账号 → await 取余额」期间账号被切换导致的错配。
    function getBalance(force = false, resolvedAccount = null) {
      const now = Date.now()
      // v0.3.3：缓存属于「某个账号的余额」。切换账号后旧快照必须立即失效，
      // 否则切过去的第一屏会显示上一个账号的余额（TTL 内不会被覆盖）。
      // 缓存键用「账号名/凭据名」而不是余额接口回来的 accountKeyRef：后者在缓存命中时读不到，
      // 会把上一个账号的快照错发给当前账号。
      const wantAccount = String(
        resolvedAccount
          ? (resolvedAccount.id || resolvedAccount.keyRef || '')
          : ((activeDeepseekAccount() || {}).keyRef || '')
      )
      if (resolvedAccount) {
        // 有明确账号时只认账本/新取的数据：宁可多刷一次，也不能把别人的余额当他的显示
        if (balanceCache && String(balanceCache.cacheKey || '') !== wantAccount) balanceCache = null
      } else if (balanceCache && String(balanceCache.cacheKey || '') !== wantAccount) {
        balanceCache = null
      }
      if (!force && balanceCache && now - balanceCache.at < BALANCE_TTL_MS) {
        return Promise.resolve(publicBalance(balanceCache.payload))
      }
      // 复用在途请求时必须确认它属于同一个账号：否则切账号后会把上一个账号正在飞的响应
      // 当成新账号的余额返回（缓存被清也救不回来，因为这条路径根本不看缓存）。（v0.3.3）
      if (balanceInFlight && balanceInFlightAccount === wantAccount) return balanceInFlight
      balanceInFlightAccount = wantAccount
      balanceInFlight = fetchBalance()
        .then((payload) => {
          if (payload.ok) {
            // 账本口径按账号分账：切到别人的 key 时，消费不会混进自己账号的「今日已用」
            const scope = String(payload.accountId || '').trim() || payload.accountTag
            recordLedgerUsage(payload.totalBalance, payload.currency, scope, Date.parse(payload.updatedAt))
            balanceCache = {
              at: Date.now(), payload,
              accountKeyRef: String(payload.accountKeyRef || ''),
              cacheKey: String(payload.accountId || payload.accountKeyRef || ''),
            }
            return publicBalance(payload)
          }
          // 已知的失败也按账号缓存：否则另一个账号的旧快照会被当成「传输抖动」沿用
    ...
    function rolesPayload() {
      ...
      return {
        ok: true,
        // v0.3.4：配色设置随角色列表一起下发（客户端换角色后立即重新取一次）
    ...
    // ===== v0.3.4：挂件配色 / 皮肤 =====
    // 目标：按钮、选中态、说明圈等「界面铬件」的颜色可以自定义，或者自动跟随角色主色。
    // 配色存进 roles.json 的 appearance 段（与角色同一份文件，所以换角色时天然能一起读）。
    // 输出的是一组 CSS 变量：客户端只负责把它们挂到 document.documentElement 上，
    // 挂件自身的 CSS 已全部写成 var(--dshw-ui, <原色>) 形式（无主题时与原样完全一致）。
    const THEME_MODES = ['auto', 'custom', 'default']
    const THEME_HEX = /^#[0-9a-fA-F]{6}$/

    function defaultAppearance() {
      return { version: 1, mode: 'auto', color: '#203170', roleId: ROLE_DEFAULT_ID }
    }
    function readAppearance() {
      let a = null
      try { a = readRolesIndex().appearance } catch (err) { a = null }
      const out = defaultAppearance()
      if (a && typeof a === 'object') {
        if (THEME_MODES.indexOf(String(a.mode)) !== -1) out.mode = String(a.mode)
        if (THEME_HEX.test(String(a.color || ''))) out.color = String(a.color).toLowerCase()
        if (a.roleId) out.roleId = String(a.roleId).slice(0, 64)
      }
      out.version = 1
      return out
    }
    function writeAppearance(patch) {
      ...
      if (THEME_MODES.indexOf(String(next.mode)) === -1) throw new Error('配色模式无效')
      if (!THEME_HEX.test(String(next.color || ''))) throw new Error('颜色必须是 #RRGGBB')
      if (next.roleId && !/^[A-Za-z0-9_-]{1,64}$/.test(String(next.roleId))) throw new Error('角色 id 无效')
      next.version = 1
      index.appearance = next
      if (!writeRolesIndex(index)) throw new Error('roles.json 写入失败')
      return next
    }
    ...
    // 由主色推导整套界面配色。选中态/强调用固定的低亮度，保证浅色泡泡里的文字始终清晰；
    // 按钮上的文字颜色按对比度自动取黑或白（WCAG 阈值 4.5）。
    function paletteFromHex(hex) {
      const rgb = hexToRgb(hex)
      if (!rgb) return null
      const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b)
      const at = (l, s) => {
        const c = hslToRgb(hsl.h, Math.max(0, Math.min(1, s == null ? hsl.s : s)), l)
        return rgbToHex(c.r, c.g, c.b)
      }
      const ui = at(0.30)
      const uiHi = at(0.42)
      const uiRgb = hexToRgb(ui)
      const fg = contrastRatio(uiRgb, { r: 255, g: 255, b: 255 }) >= 4.5 ? '#ffffff' : '#101828'
      return {
        ui, uiHi, uiFg: fg,
        txt: at(0.32, Math.min(0.8, hsl.s)),
        txtDim: at(0.62, Math.min(0.5, hsl.s * 0.8)),
        base: hex.toLowerCase(),
      }
    }

    // ---- 角色主色提取 ----
    // 优先用 sharp（快、格式全）；但 sharp 位于 DSH 安装目录，从 link: 安装的插件目录里
    // 用裸模块名 import 是解析不到的（实测 ERR_MODULE_NOT_FOUND），所以必须按「已安装位置」
    // 直接按绝对路径 import。sharp 缺失时退回自带的最小 PNG 解码器 —— 保证「跟随角色主色」
    // 在任何安装形态下都真的能工作，而不是静默变回默认蓝。
    const rolePaletteCache = new Map() // 图片路径 -> { mtimeMs, hex }
    let sharpPromise = null
    function sharpCandidates() {
      const list = []
      if (process.env.DSH_WHALE_SHARP) list.push(process.env.DSH_WHALE_SHARP)
      const home = process.env.DSH_HOME || path.join(os.homedir(), '.dsh')
      list.push(path.join(home, 'profiles', 'web', 'node_modules', 'sharp', 'dist', 'index.mjs'))
      try {
        // this file: <pkg>/lib/index.js -> node_modules/sharp (covers a normal npm install)
        list.push(path.join(PACKAGE_ROOT, '..', 'sharp', 'dist', 'index.mjs'))
        list.push(path.join(PACKAGE_ROOT, '..', '..', 'sharp', 'dist', 'index.mjs'))
      } catch (err) {}
      try {
        const req = createRequire(import.meta.url)
        const p = req.resolve('sharp')
        if (p) list.push(p)
      } catch (err) {}
      return list
    }
        ...
        sharpPromise = (async () => {
          for (const cand of sharpCandidates()) {
            try {
              ...
              const s = (m && m.default) ? m.default : m
    ...
    // 把「像素 + 简单统计」统一成主色。pickDominantHsl 由 png.mjs 提供（已单测）。
    function hexFromDominant(pixels, rgbToHslSafe) {
      const best = pickDominantHsl(pixels, rgbToHslSafe)
      if (!best) return null
      const c = hslToRgb(best.h, Math.min(1, Math.max(0.35, best.s)), Math.min(0.62, Math.max(0.32, best.l)))
      return rgbToHex(c.r, c.g, c.b)
    }
    async function dominantColorOfImage(imgPath) {
      let stat = null
      try { stat = fs.statSync(imgPath) } catch (err) { return null }
      const hit = rolePaletteCache.get(imgPath)
      if (hit && hit.mtimeMs === stat.mtimeMs) return hit.hex
      let hex = null
      // 1) sharp 路径
      try {
        const sharp = await loadSharp()
        if (sharp) {
          const { data, info } = await sharp(imgPath, { pages: 1 })
          ...
          for (let i = 0, o = 0; i + ch - 1 < data.length; i += ch, o += 4) {
            rgba[o] = data[i]
            rgba[o + 1] = ch >= 3 ? data[i + 1] : data[i]
            rgba[o + 2] = ch >= 3 ? data[i + 2] : data[i]
            rgba[o + 3] = ch >= 4 ? data[i + 3] : (ch === 2 ? data[i + 1] : 255)
          }
          hex = hexFromDominant(rgba, rgbToHsl)
        }
      } catch (err) { hex = null }
      // 2) 自带 PNG 解码器兜底（角色 png 一律是 8bit 非隔行）
      if (!hex) {
        try {
          const decoded = decodePngToRgba(fs.readFileSync(imgPath))
          if (decoded) hex = hexFromDominant(decoded.pixels, rgbToHsl)
        } catch (err) { hex = null }
      }
      rolePaletteCache.set(imgPath, { mtimeMs: stat.mtimeMs, hex })
      return hex
    }

    // 当前生效的角色 → 主色 → 整套配色。mode='auto' 跟随角色，'custom' 用自选色，'default' 回落内置蓝。
    async function resolveAppearance() {
      const a = readAppearance()
      if (a.mode === 'default') {
        return { ok: true, mode: a.mode, color: '#203170', roleId: '', palette: paletteFromHex('#203170'), source: 'default' }
      }
      if (a.mode === 'custom') {
        const p = paletteFromHex(a.color)
        return p
          ? { ok: true, mode: a.mode, color: a.color, roleId: '', palette: p, source: 'custom' }
          : { ok: false, error: '颜色无效' }
      }
      // auto：角色图主色；取不到就回落内置蓝（并说明原因）
      const roleId = a.roleId || ROLE_DEFAULT_ID
      const imgPath = roleId === ROLE_DEFAULT_ID
        ? (IMAGE_CANDIDATES.find((p) => { try { return fs.existsSync(p) } catch (e) { return false } }) || IMAGE_CANDIDATES[0])
        : roleImagePath(roleId)
      let hex = null
      if (imgPath) hex = await dominantColorOfImage(imgPath)
      if (!hex) return { ok: true, mode: a.mode, color: '#203170', roleId, palette: paletteFromHex('#203170'), source: 'fallback' }
      return { ok: true, mode: a.mode, color: hex, roleId, palette: paletteFromHex(hex), source: 'role' }
    }
      ...
      handler: async (req, res) => {
        try {
          if (req.method === 'PUT' || req.method === 'POST') {
            ...
            // v0.3.3：DeepSeek 账号切换 / 新增改名 / 删除（内置鲸鱼用哪把 key 查余额）
            if (action === 'set-account') {
              const r = setActiveDeepseekAccount(parsed.accountId)
              const listA = await apiModelsPayload()
              res.writeHead(r.ok ? 200 : 400, JSON_HEADERS)
              res.end(JSON.stringify(Object.assign({}, r, { models: listA.models, accounts: listA.accounts })))
              return
            }
            if (action === 'save-account') {
              const r = await apiSaveAccount(parsed.account || parsed)
              const listB = await apiModelsPayload()
              res.writeHead(r.ok ? 200 : 400, JSON_HEADERS)
              res.end(JSON.stringify(Object.assign({}, r, { models: listB.models, accounts: listB.accounts })))
              return
            }
            if (action === 'delete-account') {
              const r = apiDeleteAccount(parsed)
              const listC = await apiModelsPayload()
              res.writeHead(r.ok ? 200 : 400, JSON_HEADERS)
              res.end(JSON.stringify(Object.assign({}, r, { models: listC.models, accounts: listC.accounts })))
              return
            }
    ...
    disposers.push(registerRoute({
      kind: 'exact',
      path: '/dsh-whale/appearance.json',
      handler: async (req, res) => {
        try {
          if (req.method === 'POST' || req.method === 'PUT') {
            ...
            const parsed = JSON.parse(body || '{}')
            const patch = {}
            if (parsed.mode !== undefined) patch.mode = String(parsed.mode)
            if (parsed.color !== undefined) patch.color = String(parsed.color)
            if (parsed.roleId !== undefined) patch.roleId = String(parsed.roleId)
