...
export default {
  ...
  apply(ctx) {
    ...
    function rejected(req, res) {
      ...
      const self = selfRejection(req)
      if (self !== null) return deny(self)
      const conn = connectionFence()
      if (!conn) return false // ① 已通过；此刻没有宿主栅栏可委托（加载时已 warn）
      let code
      try {
    ...
    function usageRecordsPayload() {
      ...
      return {
        ok: true, version: '0.3.11', today: todayData, days7,
    ...
    function publicBalance(payload) {
      ...
      return {
        ...visible, version: '0.3.11', isPeak: isPeakTime(nowSec),
    ...
    // 桌面客户端（DSH Desktop / Electron）的主窗口加载 dsh-app://app/，index.html 来自
    // App 包内静态 dist，插件注入只应用 `webserver/index-inject` 的结构化注入行
    // （global / script / script-src / script-preload / style / html）。上面的 tapIndex
    // 是"原始 HTML 变换"，只由宿主自渲染 index 时应用（即浏览器访问 dsh web 的场景），
    // collectIndexInjections() 不收集它 —— 于是桌面端拿不到 widget.js，挂件不显示。
    // 这里补一行结构化注入：renderIndex() 在浏览器端同样渲染结构化行，两种形态都覆盖；
    // 重复注入由前端 `if (window.__dshWhaleWidget) return` 幂等守卫兜住；
    // 旧宿主不发出该事件时监听器不会被调用，行为与现状一致。
    disposers.push(ctx.on('webserver/index-inject', (table) => {
      if (!Array.isArray(table)) return
      if (table.some((row) => row && row.kind === 'script-src' && row.src === '/dsh-whale/widget.js')) return
      table.push({ kind: 'script-src', placement: 'head', src: '/dsh-whale/widget.js' })
    }))
    ctx.effect(() => () => {
