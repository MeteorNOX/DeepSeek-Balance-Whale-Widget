...
function onDocPointerCancel(e) { // 手势被浏览器接管/系统侧边手势中断：留在当前位置，按现位贴边归位并保存（不再弹回起点）
  if (drag && drag.active) { drag.cancelled = true; snapCheck(); settle(); saveConfig() }
  endDrag(e, false)
}
...
function endDrag(e, clickAllowed) {
  ...
  if (drag.cancelled) { snapCheck(); settle(); saveConfig(); drag.cancelled = false; return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
