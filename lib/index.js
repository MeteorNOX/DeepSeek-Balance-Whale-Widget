...
const SOUND_SETS = {
  ...
  cat: {
    press: [path.join(PACKAGE_ROOT, 'assets', 'E1.mp3'), 'D:/TestBox/deepseek/skin/E1.mp3'],
    release: [path.join(PACKAGE_ROOT, 'assets', 'E2.mp3'), 'D:/TestBox/deepseek/skin/E2.mp3'],
  },
  nlong: {
    press: [path.join(PACKAGE_ROOT, 'assets', 'N1.mp3'), 'D:/TestBox/deepseek/skin/N1.mp3'],
    release: [path.join(PACKAGE_ROOT, 'assets', 'N2.mp3'), 'D:/TestBox/deepseek/skin/N2.mp3'],
  },
}
...
var css = [
  ...
  '.dshwv-img{position:absolute;right:0;bottom:0;width:59.45%;height:59.45%;display:block;pointer-events:none;-webkit-user-drag:none;user-select:none;object-fit:contain}',
  ...
  '.dshwv-img{animation:dshwv-bob 2.8s ease-in-out infinite}',
  '@keyframes dshwv-bob{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-1.5%) scale(1.012)}}'
].join('\\n')
...
var imgsrc = function (skin) {
  return '/dsh-whale/image.png' + (skin && skin !== 'default' ? '?skin=' + encodeURIComponent(skin) + '&v=' + Date.now() : '?v=' + Date.now())
}
var currentSkin = localStorage.getItem('dshwv-skin') || 'default'
img.src = imgsrc(currentSkin)
img.alt = 'DeepSeek 余额'
img.draggable = false
...
soundSelect.appendChild(soundOpt('cat', '猫喵'))
soundSelect.appendChild(soundOpt('nlong', '奶龙大笑'))
...
skinSelect.appendChild(soundOpt('default', '鲸鱼娘 (默认)'))
skinSelect.appendChild(soundOpt('tiaotiao', '跳跳喵'))
skinSelect.appendChild(soundOpt('naillong', '奶龙'))
skinSelect.appendChild(soundOpt('naiwa', '奶龙·大笑'))
skinSelect.value = currentSkin
skinSelect.addEventListener('change', function () {
  currentSkin = skinSelect.value
  localStorage.setItem('dshwv-skin', currentSkin)
  img.src = imgsrc(currentSkin)
})
...
var skinRow = menuRow()
skinRow.appendChild(menuLabel('形象'))
...
function setSoundSet(v) {
  soundSet = (v === 'fx1' || v === 'cat' || v === 'nlong') ? v : 'duck'
...
function apply(ctx) {
    ...
    function loadImage(skin) {
      if (skin) {
        const sp = path.join(PACKAGE_ROOT, 'assets', 'skin', String(skin), 'DSniang1.png')
        try {
