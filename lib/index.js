...
export default {
  ...
  apply(ctx) {
    ...
    function readUsageLedger() {
      ...
      return { date: todayKey(), lastBalance: null, balanceSpent: 0, todayUsage: 0, history: {} }
    }
    ...
    // 内置 DeepSeek 的余额差记账：累计每次观测到的余额下降，而不是 dayStart - lastBalance。
    // 后者在当天充值/退款后会把已经发生的消费冲掉。旧账本没有 balanceSpent 时，
    // 只能用原来的净余额差作一次兼容回退；从下一次成功余额观测起就会转为累计口径。
    function ledgerBalanceSpent(led) {
      if (led && Object.prototype.hasOwnProperty.call(led, 'balanceSpent')) {
        const spent = Number(led.balanceSpent)
        return isFinite(spent) && spent > 0 ? spent : 0
      }
      if (led && typeof led.dayStart === 'number' && typeof led.lastBalance === 'number') {
        return Math.max(0, led.dayStart - led.lastBalance)
      }
      return 0
    }
    ...
    function usageRecordsPayload() {
      ...
      // 当日真实消费 = 累计观测到的余额下降；充值/退款不会冲减这个值。
      function realDelta() {
        return ledgerBalanceSpent(led)
      }
    ...
    // 记账：累计每次余额下降。充值/退款只会抬高 lastBalance，不会冲减此前的消费；
    // 余额差不可用时才用会话事件兜底。
    function recordLedgerUsage(currentBalance) {
      ...
      function sumEv(d) { let s = 0; for (const e of events) if (e.day === d) s += Number(e.cost) || 0; return s }
      if (led.date !== t) {
        // 跨天：有累计余额下降时它就是权威总额；尚未观测到下降时才退回事件/历史。
        const oldDay = led.date
        if (oldDay) {
          led.history = led.history || {}
          const spent = ledgerBalanceSpent(led)
          let oldTotal = spent > 0.004 ? spent : sumEv(oldDay)
          if (oldTotal <= 0.004 && typeof led.todayUsage === 'number') oldTotal = led.todayUsage
        ...
        // 兼容旧账本：首次写入时把旧净余额差冻结为起点；之后只累计新增下降。
        if (!Object.prototype.hasOwnProperty.call(led, 'balanceSpent')) led.balanceSpent = ledgerBalanceSpent(led)
        if (typeof led.dayStart !== 'number' && typeof led.lastBalance === 'number') {
          led.dayStart = led.lastBalance
        }
        if (typeof led.lastBalance === 'number' && currentBalance < led.lastBalance - 0.000001) {
          led.balanceSpent = (Number(led.balanceSpent) || 0) + (led.lastBalance - currentBalance)
        }
        led.lastBalance = currentBalance
      }
    ...
    function ledgerTodayTotal(led) {
      ...
      const spent = ledgerBalanceSpent(led)
      if (spent > 0.004) return round2(spent)
      return round2(s)
    }
