...
function onDocPointerCancel(e) { // 手势被浏览器接管/系统中断：还原拖拽起点，不归边不保存
  if (drag && drag.active) { drag.cancelled = true; state.left = drag.origLeft; state.top = drag.origTop; settle() }
  endDrag(e, false)
}
...
function endDrag(e, clickAllowed) {
  ...
  if (drag.cancelled) { state.left = drag.origLeft; state.top = drag.origTop; settle(); drag.cancelled = false; return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
