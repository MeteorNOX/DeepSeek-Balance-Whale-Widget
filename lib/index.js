...
// GLM Coding Plan 用量（智谱官方配额接口，与官方 Claude Code 插件 glm-plan-usage
// 的 usage-query 同源；仅支持个人版套餐）。凭据走 DSH 凭据服务的 ZAI_CODING_CN_API_KEY。
// Docs: https://docs.bigmodel.cn/cn/coding-plan/extension/usage-query-plugin
const GLM_USAGE_URL = 'https://open.bigmodel.cn/api/monitor/usage/quota/limit'
const GLM_USAGE_TTL_MS = 30000
const GLM_CREDENTIAL = 'ZAI_CODING_CN_API_KEY'

// 挂件显示域：auto=跟随 DSH 当前默认模型自动切换（DeepSeek 模型→余额，GLM 模型→用量）；
// glm/deepseek=手动指定。模型识别优先读运行中的 agentDefaultModel 服务，回退解析
// settings.yaml 的 agent-default-model 段。
const DISPLAY_MODES = ['auto', 'glm', 'deepseek']
function normalizeDisplayMode(m) {
  return DISPLAY_MODES.indexOf(m) !== -1 ? m : 'auto'
}
function classifyProvider(provider, model) {
  const p = String(provider || '').toLowerCase()
  const m = String(model || '').toLowerCase()
  if (/deepseek/.test(m) || /deepseek/.test(p)) return 'deepseek'
  if (/glm/.test(m) || /zai|zhipu|bigmodel/.test(p)) return 'glm'
  return 'unknown'
}
function detectModelSelection(ctx) {
  // 1) DSH 运行中的 agentDefaultModel 服务（实时反映默认模型选择）
  try {
    const svc = typeof ctx.get === 'function' ? ctx.get('agentDefaultModel') : null
    const sel = svc && typeof svc.currentSelection === 'function' ? svc.currentSelection() : null
    if (sel && sel.model) return { provider: sel.provider, model: sel.model, source: 'live' }
  } catch (err) {}
  // 2) 回退：解析 $DSH_HOME/settings.yaml 的 agent-default-model 段
  try {
    const lines = fs.readFileSync(path.join(DSH_HOME, 'settings.yaml'), 'utf8').split(/\r?\n/)
    const idx = lines.findIndex((l) => /^agent-default-model:/.test(l))
    if (idx >= 0) {
      let provider = null
      let model = null
      for (let i = idx + 1; i < lines.length; i++) {
        if (!/^\s/.test(lines[i])) break
        let mm = lines[i].match(/^\s+provider:\s*(\S+)/)
        if (mm && provider === null) provider = mm[1]
        mm = lines[i].match(/^\s+model:\s*(\S+)/)
        if (mm && model === null) model = mm[1]
      }
      if (model) return { provider, model, source: 'settings' }
    }
  } catch (err) {}
...
var GLM_USAGE_URL = '/dsh-whale/glm-usage.json'
var PROVIDER_URL = '/dsh-whale/provider.json'
var SIZE_URL = '/dsh-whale/size.json'
var IMG_URL = '/dsh-whale/image.png?v=2'
var GIF_URL = '/dsh-whale/rua.gif'
var DISPLAY_MODES = ['auto', 'glm', 'deepseek']
...
var css = [
  ...
  '.dshwv-root.dshwv-glm .dshwv-text{color:#2e7d6b}',
  '.dshwv-root.dshwv-glm .dshwv-hint{color:#9cb8ad}',
...
displaySelect.appendChild(soundOpt('auto', '自动 (跟随模型)'))
displaySelect.appendChild(soundOpt('glm', 'GLM 用量'))
displaySelect.appendChild(soundOpt('deepseek', 'DeepSeek 余额'))
displaySelect.addEventListener('change', function () { setDisplayMode(displaySelect.value) })
var glmViewSelect = document.createElement('select')
glmViewSelect.className = 'dshwv-sound'
glmViewSelect.appendChild(soundOpt('five', '5 小时窗口'))
glmViewSelect.appendChild(soundOpt('week', '本周窗口'))
glmViewSelect.addEventListener('change', function () { setGlmView(glmViewSelect.value) })
...
var rowDisp = menuRow()
rowDisp.appendChild(menuLabel('显示'))
rowDisp.appendChild(displaySelect)
var rowGlm = menuRow()
rowGlm.appendChild(menuLabel('GLM窗口'))
rowGlm.appendChild(glmViewSelect)
var row6 = menuRow()
...
menuBox.appendChild(rowDisp)
menuBox.appendChild(rowGlm)
...
var shown = null
var animId = null
// GLM 显示域状态：displayMode 记忆在 size config（服务端持久化），
// effDisplay 是应用后的实际显示域（glm | deepseek）
var displayMode = 'auto'
var effDisplay = 'deepseek'
var glmView = 'five'
var glmData = null
var glmStatus = 'loading'
var glmMessage = ''
...
function fmtPct(v) {
  var num = Number(v)
  return isFinite(num) ? Math.round(num) + '%' : '--'
}
...
function fmtCountdown(nextMs) {
  var t = Number(nextMs)
  if (!nextMs || !isFinite(t)) return '--:--'
  var diff = t - Date.now()
...
function glmSeg() {
  if (!glmData || !glmData.ok) return null
  return glmView === 'five' ? (glmData.fiveHour || null) : (glmData.weekly || null)
}
function glmPct() {
  var seg = glmSeg()
  if (!seg) return null
  var p = Number(seg.percentage)
  return isFinite(p) ? p : null
}
function glmHint() {
  if (!glmData || !glmData.ok) return ''
  var seg = glmSeg()
  if (!seg) return glmView === 'five' ? '暂无5小时额度' : '套餐无周额度'
  var parts = []
  if (glmView === 'week' && seg.usage !== null && seg.usage !== undefined && seg.currentValue !== null && seg.currentValue !== undefined) {
    parts.push(fmtTokens(seg.currentValue) + '/' + fmtTokens(seg.usage))
  }
  var cd = fmtCountdown(seg.nextResetTime)
  if (cd) parts.push('重置 ' + cd)
  return parts.length ? parts.join(' · ') : '点击刷新'
}
function glmLabel() {
  return glmView === 'five' ? 'GLM 5小时' : 'GLM 本周'
}
function animateAmount(from, to, currency, duration, fmtFn) {
  // 消耗金额泡泡显示期间，余额数字滚动不触碰金额行
  if (costBubbleActive) return
  var fmtOf = fmtFn || function (v) { return fmt(v, currency) }
  if (animId) cancelAnimationFrame(animId)
  if (from === null || !isFinite(from)) from = to
  if (from === to) {
    shown = to
    amountEl.textContent = fmtOf(to)
    return
  }
  var startTime = null
  function step(ts) {
    ...
    amountEl.textContent = fmtOf(val)
    if (t < 1) {
...
function render() {
  ...
  var amount, hint
  if (effDisplay === 'glm') {
    // GLM Coding Plan 用量显示域：5 小时 / 本周 token 百分比 + 重置倒计时
    root.classList.add('dshwv-glm')
    labelEl.textContent = glmLabel()
    if (glmStatus === 'error') {
      amount = shown !== null ? fmtPct(shown) : '--'
      hint = glmMessage ? glmMessage.slice(0, 14) : '获取失败 · 点击重试'
    } else if (glmData === null) {
      amount = shown !== null ? fmtPct(shown) : '…'
      hint = '加载中…'
    } else {
      var gp = glmPct()
      amount = gp === null ? '--' : fmtPct(shown !== null ? shown : gp)
      hint = glmHint()
    }
  ...
  root.classList.remove('dshwv-glm')
  labelEl.textContent = 'DeepSeek 余额'
...
function refresh(manual) {
  if (busy) return
  busy = true
  // 先同步当前模型识别结果（决定显示域），再按域拉取对应数据
  fetch(PROVIDER_URL, { cache: 'no-store' })
    .then(function (r) { return r.json() })
    .then(function (d) { applyProviderData(d) })
    .catch(function () {})
    .then(function () {
      render()
      return effDisplay === 'glm' ? loadGlmData(manual) : loadBalanceData(manual)
    })
    .then(function () { busy = false }, function () { busy = false })
}
function applyProviderData(d) {
  if (!(d && d.ok)) return
  displayMode = DISPLAY_MODES.indexOf(d.mode) !== -1 ? d.mode : 'auto'
  displaySelect.value = displayMode
  var eff = d.provider === 'glm' ? 'glm' : 'deepseek'
  // 显示域切换时重置动画基准值，避免「¥金额 → 百分比」的数字滚动串域
  if (eff !== effDisplay) shown = null
  effDisplay = eff
}
function loadBalanceData(manual) {
  if (animDelayTimer) { clearTimeout(animDelayTimer); animDelayTimer = null }
...
function loadGlmData(manual) {
  if (manual || glmData === null) { glmStatus = 'loading'; render() }
  var ctrl = null
  var timer = null
  try {
    ctrl = new AbortController()
  ...
  return fetch(GLM_USAGE_URL, { cache: 'no-store', signal: ctrl ? ctrl.signal : undefined })
    .then(function (r) { return r.json() })
    .then(function (data) {
      if (data && data.ok) {
        var seg = glmView === 'five' ? data.fiveHour : data.weekly
        var target = seg && isFinite(Number(seg.percentage)) ? Number(seg.percentage) : null
        var prev = shown
        glmData = data
        glmStatus = 'ok'
        glmMessage = ''
        if (target === null) {
          // 当前窗口无配额段（如旧套餐无周额度）或字段异常：置空走 '--' 展示
          shown = null
          render()
        } else if (prev === null || prev === target || !isFinite(prev)) {
          shown = target
          render()
        } else {
          animateAmount(prev, target, '', ANIM_MS, fmtPct)
          render()
        }
      } else {
        glmStatus = 'error'
        glmMessage = (data && data.error) ? String(data.error) : '获取失败'
        render()
      }
    })
    .catch(function () {
...
function setDisplayMode(v) {
  displayMode = DISPLAY_MODES.indexOf(v) !== -1 ? v : 'auto'
  displaySelect.value = displayMode
  saveConfig()
  // 立即按新模式拉取：auto 下服务端返回的 provider 决定实际显示域
  refresh(true)
}
function setGlmView(v) {
  glmView = v === 'week' ? 'week' : 'five'
  glmViewSelect.value = glmView
  // 两窗口数据同源（glmData 同时含 fiveHour/weekly），本地切换无需重新请求
  shown = null
  saveConfig()
  render()
}
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.displayMode === 'string') {
      displayMode = DISPLAY_MODES.indexOf(d.displayMode) !== -1 ? d.displayMode : 'auto'
      displaySelect.value = displayMode
    }
    if (d && typeof d.glmView === 'string') {
      glmView = d.glmView === 'week' ? 'week' : 'five'
      glmViewSelect.value = glmView
    }
...
function apply(ctx) {
    ...
    // GLM Coding Plan 用量：官方配额接口（与 glm-plan-usage 的 usage-query 同源）。
    // limits 里 TOKENS_LIMIT unit=3 为 5 小时滚动窗口，unit=6 为本周窗口（新版套餐
    // 才返回），TIME_LIMIT 为 MCP/工具调用额度（顺带返回，客户端暂不展示）。
    async function fetchGlmUsage() {
      let cred
      try {
        cred = await ctx.credentials.resolve(GLM_CREDENTIAL)
      } catch (err) {
        return { ok: false, code: 'NO_KEY', error: '凭据读取失败: ' + String((err && err.message) || err).slice(0, 160) }
      }
      if (!cred) {
        return { ok: false, code: 'NO_KEY', error: '未配置 ' + GLM_CREDENTIAL }
      }
      let lastErr = null
      for (let attempt = 0; attempt < 2; attempt++) {
        let res
        try {
          res = await fetch(GLM_USAGE_URL, {
            headers: { Authorization: 'Bearer ' + cred.value },
            signal: AbortSignal.timeout(20000),
          })
        } catch (err) {
        ...
        if (!res.ok) {
          if (res.status === 401 || res.status === 403) {
            return { ok: false, code: 'AUTH', transient: false, error: 'GLM Key 无效或无权限(HTTP ' + res.status + ')' }
          }
          lastErr = new Error('HTTP ' + res.status)
          if (res.status < 500) break
          if (attempt === 0) await new Promise((r) => setTimeout(r, 500))
        ...
        try {
          ...
          return { ok: false, code: 'PARSE', error: 'GLM 用量接口返回不是合法 JSON' }
        }
        const limits = data && data.data && Array.isArray(data.data.limits) ? data.data.limits : null
        if (!limits) {
          return { ok: false, code: 'SHAPE', error: 'GLM 用量接口返回结构异常' }
        }
        const pick = (type, unit) => limits.find((l) => l && l.type === type && l.unit === unit) || null
        const t5 = pick('TOKENS_LIMIT', 3)
        const tw = pick('TOKENS_LIMIT', 6)
        const tm = pick('TIME_LIMIT', 5)
        return {
          ok: true,
          plan: String((data.data && data.data.level) || ''),
          fiveHour: t5 ? { percentage: Number(t5.percentage), nextResetTime: t5.nextResetTime ?? null } : null,
          weekly: tw ? {
            percentage: Number(tw.percentage),
            usage: tw.usage ?? null,
            currentValue: tw.currentValue ?? null,
            nextResetTime: tw.nextResetTime ?? null,
          } : null,
          mcp: tm ? {
            usage: Number(tm.usage),
            currentValue: Number(tm.currentValue),
            remaining: Number(tm.remaining),
            percentage: Number(tm.percentage),
            nextResetTime: tm.nextResetTime ?? null,
          } : null,
          updatedAt: new Date().toISOString(),
        }
      }
      ...
      return {
        ...
        code: 'HTTP',
        transient: transient,
        error: 'GLM 用量请求失败: ' + String((lastErr && lastErr.message) || lastErr).slice(0, 200),
      }
    }

    function getGlmUsage() {
      const now = Date.now()
      if (glmCache && now - glmCache.at < GLM_USAGE_TTL_MS) {
        return Promise.resolve(glmCache.payload)
      }
      if (glmInFlight) return glmInFlight
      glmInFlight = fetchGlmUsage()
        .then((payload) => {
          if (payload.ok) {
            glmCache = { at: now, payload }
            return payload
          }
          if (payload.transient && glmCache) {
            return { ...glmCache.payload, stale: true, error: payload.error }
          }
          if (!payload.transient) console.error('[whale-glm]', payload.code, payload.error)
          return payload
        })
        .catch((err) => ({
          ok: false,
          code: 'ERROR',
          error: 'GLM 用量服务异常: ' + String((err && err.message) || err).slice(0, 200),
        }))
        .finally(() => {
          glmInFlight = null
        })
    ...
    // 当前显示模式与模型识别结果：mode 为用户选择（size config 持久化），
    // provider 为应用后的实际显示域（auto 时由检测到的模型归属决定）
    function providerPayload() {
      const cfg = readSizeConfig() || {}
      const mode = normalizeDisplayMode(cfg.displayMode)
      const sel = detectModelSelection(ctx)
      const detected = classifyProvider(sel.provider, sel.model)
      const provider = mode === 'glm' ? 'glm' : mode === 'deepseek' ? 'deepseek' : (detected === 'glm' ? 'glm' : 'deepseek')
      return { ok: true, mode, detected, provider, model: sel.model || '', source: sel.source }
    }
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              glmView: parsed.glmView === 'week' ? 'week' : 'five',
            }
          }
        } catch (err) {}
      }
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, displayMode, glmView) {
      ...
      const dm = normalizeDisplayMode(displayMode)
      const gv = glmView === 'week' ? 'week' : 'five'
    ...
    // GLM Coding Plan 用量（5 小时 / 本周窗口），凭据 ZAI_CODING_CN_API_KEY
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/glm-usage.json',
      handler: async (req, res) => {
        try {
          const payload = await getGlmUsage()
          res.writeHead(200, JSON_HEADERS)
          res.end(JSON.stringify(payload))
        } catch (err) {
          res.writeHead(200, JSON_HEADERS)
    ...
    // 显示模式与模型识别：GET 返回 { mode, detected, provider, model, source }
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/provider.json',
      handler: (req, res) => {
        res.writeHead(200, JSON_HEADERS)
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.displayMode, parsed.glmView)
