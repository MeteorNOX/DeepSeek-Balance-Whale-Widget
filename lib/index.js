...
// 点击挂件时可以切换到的额外素材（本体之外的表情图）。
// 每一项 = 一个固定路由 + 若干候选文件名（大小写都试，兼容 Windows 上生成的 .PNG）。
// 新增图片：把文件放进 assets/，在这里加一项，再到前端 EXTRA_IMG_URLS 加一行。
const EXTRA_IMAGES = [
...
// 定时「吃饭」动画素材：VP9 + alpha，610×610，122 帧 / 4.07s。
// 用 <video> 播放（img 不能播视频）；解码失败或环境不支持 VP9 alpha 时
// 整段功能静默停用，挂件保持本体静态图，不出现破图/黑块。
const EATING_VIDEO_CANDIDATES = [
  path.join(PACKAGE_ROOT, 'assets', 'DSniang_eating_rice_2.webm'),
  path.join(PACKAGE_ROOT, 'assets', 'DSniang_eating_rice_2.WEBM'),
  'D:/TestBox/deepseek/skin/DSniang_eating_rice_2.webm',
  'D:/TestBox/deepseek/DSniang_eating_rice_2.webm',
]
// 吃饭动画触发间隔（分钟）默认值；0 = 关闭
const DEFAULT_EAT_MIN = 15
// 允许的间隔档位（菜单里也按这组渲染）；非法值一律回落到默认 15 分钟
const EAT_MIN_CHOICES = [0, 5, 15, 30, 60]
function normalizeEatMin(v) {
  const n = Number(v)
  for (const c of EAT_MIN_CHOICES) {
    if (n === c) return c
  }
  return DEFAULT_EAT_MIN
}
...
var EAT_URL = '/dsh-whale/eating.webm'
// 「吃饭」动画：淡入淡出时长 / 定时检查周期 / 间隔档位（与后端 EAT_MIN_CHOICES 一致）
var EAT_FADE_MS = 240
var EAT_CHECK_MS = 60000
var EAT_MIN_CHOICES = [0, 5, 15, 30, 60]
var DEFAULT_EAT_MIN = 15
// 额外素材（各种表情）：点击挂件时按顺序循环切换，见 rollWhaleImage()。
// 顺序 = 本体(DSniang1) -> 下面这几张（按数组顺序）-> 回到本体。
// 新增素材：assets/ 放文件 -> 真机侧 EXTRA_IMAGES 与调试台 EXTRA_IMAGES 加一项 -> 这里加一行。
var EXTRA_IMG_URLS = [
  '/dsh-whale/image-tired.png?v=2',
  '/dsh-whale/image-mad.png?v=2',
  '/dsh-whale/image-sad.png?v=2',
]
...
var css = [
  ...
  // transition 时长与 .dshwv-eat / EAT_FADE_MS(240ms) 对齐：两层同频交叉溶解。
  '.dshwv-img{position:absolute;right:0;bottom:0;width:59.45%;height:59.45%;display:block;pointer-events:none;-webkit-user-drag:none;user-select:none;transition:opacity .24s ease}',
  // 「吃饭」播放期间隐藏底下这张本体图，否则视频角色位移时会露出它的硬边。
  // 类挂在 .dshwv-body 上（不是 document.body）：body 上有 pressDown() 的 squish
  // transform，动 classList 最安全，不碰 body.style。退出时（abort/finish）移除。
  '.dshwv-eat-mode .dshwv-img{opacity:0}',
  // 「吃饭」动画视频：与 .dshwv-img 完全同位同尺寸的叠层，常态透明（opacity 0）。
  // 镜像不用额外规则：它是 .dshwv-body 的子元素，随 .dshwv-root.dshwv-left 的
  // scaleX(-1) 一起翻转（.dshwv-gif 单独写是因为它自带 translate 需要重写）。
  '.dshwv-eat{position:absolute;right:0;bottom:0;width:59.45%;height:59.45%;display:block;pointer-events:none;-webkit-user-drag:none;user-select:none;opacity:0;transition:opacity .24s ease}',
...
img.alt = 'DeepSeek 余额'
img.draggable = false
img.addEventListener('error', revertWhaleImage)

// —— 点击挂件时在「本体」与 EXTRA_IMG_URLS 之间按顺序循环切换 ——
// whaleIdx: 0 = 本体，i>0 = EXTRA_IMG_URLS[i-1]；extraSrcs[i] 是预取到的 blob URL。
var extraSrcs = EXTRA_IMG_URLS.map(function () { return '' })
var whaleIdx = 0
// 预取全部额外素材并转成 blob URL：素材较大（~2MB），这样每张只下载一次，
// 之后点击切换都是本地内存 URL，零延迟，也不会每次点击都重新走网络。
// 真机 webview 里 fetch 可能被 CSP 拦、或预取还没回来，都不影响切换 —— 会退回直链。
EXTRA_IMG_URLS.forEach(function (url, i) {
  try {
    fetch(url, { cache: 'force-cache' })
      .then(function (r) { if (!r.ok) throw new Error('extra image ' + r.status); return r.blob() })
      .then(function (b) { extraSrcs[i] = URL.createObjectURL(b) })
      .catch(function () {})
  } catch (err) {}
})
function rollWhaleImage() {
  // 按顺序循环：本体 -> 疲惫 -> 生气 -> 难过 -> 回到本体……
  // 每次点击都是下一张，保证画面一定变化，顺序可预期。
  var total = 1 + EXTRA_IMG_URLS.length
  whaleIdx = (whaleIdx + 1) % total
  try { img.src = whaleIdx === 0 ? IMG_URL : (extraSrcs[whaleIdx - 1] || EXTRA_IMG_URLS[whaleIdx - 1]) } catch (err) {}
}
// 素材加载失败（例如文件缺失）时立刻退回本体，避免出现破图
function revertWhaleImage() {
  if (whaleIdx === 0) return
  whaleIdx = 0
...
// 「吃饭」：定时把本体换成一段吃饭动画，完整播完一遍后自动切回静态图（0 = 关）
var eatMinSelect = document.createElement('select')
eatMinSelect.className = 'dshwv-sound'
eatMinSelect.title = '每隔一段时间把挂件换成吃饭动画，完整播一遍后自动切回'
eatMinSelect.appendChild(soundOpt('0', '关'))
eatMinSelect.appendChild(soundOpt('5', '5 分钟'))
eatMinSelect.appendChild(soundOpt('15', '15 分钟'))
eatMinSelect.appendChild(soundOpt('30', '30 分钟'))
eatMinSelect.appendChild(soundOpt('60', '60 分钟'))
eatMinSelect.value = String(DEFAULT_EAT_MIN)
eatMinSelect.addEventListener('change', function () { setEatMin(eatMinSelect.value) })
var row1 = menuRow()
...
var row8 = menuRow()
row8.appendChild(menuLabel('吃饭'))
row8.appendChild(eatMinSelect)
var row9 = menuRow()
...
bubbleBox.addEventListener('click', function (e) {
  ...
  // 点击气泡与点击 DSniang 行为一致：显示中 → 换随机内容
  clickWhale()
})
...
var RANDOM_GROUPS = [
  ...
  { w: 20, lines: function () { return singleCenter('A', pickOne(['Abq是好主人...', 'Abq是大笨蛋...↓'])) } },
  { w: 10, lines: function () { return singleCenter('A', pickOne(['不知道Abq有什么用，还得是我~', '挣到钱了吗？', '是啊，吃什么呢', '想我了？！', 'DeepSleep...', '又让我干活！']), '', true) } },
  { w: 10, lines: function () { return { gif: true } } },
  { w: 20, lines: function () { return singleCenter('A', pickOne(['你目录里的dsh是什么...大烧货吗...?', '恭喜你实现token自由！token全被我吃掉了！', 'Abq当我是便宜货啊...']), '', true) } },
  { w: 20, lines: function () { return singleCenter('B', '❤Bro... ') } },
]
...
// 气泡空闲自动关闭：每次展示/交互都重新计时，实现「不操作 5 秒后自动收起」
function scheduleBubbleClose() {
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
  bubbleTimer = setTimeout(hideBubble, BUBBLE_MS)
}
// 点击 DSniang：气泡未显示时先显示余额；气泡显示期间再次点击则换一段随机内容
// （即 RANDOM_GROUPS 的随机台词/动图），并重置自动关闭计时，保证新内容有完整停留时间。
function clickWhale() {
  // 播放中的「吃饭」动画先中断、切回本体静态图，再走原本的点击逻辑 ——
  // 否则动画会卡在中途，点击想看的换脸效果也被它盖住。
  abortEating()
  // 每次点击都切换素材；放在早退之前，保证关了气泡开关时点击同样有效
  rollWhaleImage()
  if (!bubbleOn) return
  // 消耗金额泡泡显示期间不响应点击（与 showBubble 的早退保持一致）
...
function showBubble() {
  ...
  // 默认展示余额；再次点击 DSniang/气泡切到随机台词段；不操作 5 秒后自动关闭
  scheduleBubbleClose()
}
function hideBubble() {
  if (bubbleTimer) { clearTimeout(bubbleTimer); bubbleTimer = null }
...
// 只认 EAT_MIN_CHOICES 里的档位，其余（含旧配置里的怪值）一律回落默认 15 分钟
function setEatMin(v) {
  var n = Number(v)
  var hit = null
  for (var i = 0; i < EAT_MIN_CHOICES.length; i++) {
    if (EAT_MIN_CHOICES[i] === n) hit = EAT_MIN_CHOICES[i]
  }
  eatMin = hit === null ? DEFAULT_EAT_MIN : hit
  eatMinSelect.value = String(eatMin)
  // 关掉时若正在播，立刻切回本体
  if (eatMin <= 0) abortEating()
  // 改完重新计时，避免刚选完就立刻播一次
  lastEatAt = Date.now()
  saveConfig()
}
...
function endDrag(e, clickAllowed) {
  ...
  if (clickAllowed && !drag.moved) { clickWhale(); return }
  var dx = e.clientX - drag.startX
  var dy = e.clientY - drag.startY
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.eatMin === 'number') {
      // 这里只做白名单归一化，不调 setEatMin：那个会 saveConfig() 回写，
      // 启动时多一次 PUT 没必要（服务端读配置时已经规范化过了）。
      var eatHit = null
      for (var eIdx = 0; eIdx < EAT_MIN_CHOICES.length; eIdx++) {
        if (EAT_MIN_CHOICES[eIdx] === d.eatMin) eatHit = EAT_MIN_CHOICES[eIdx]
      }
      eatMin = eatHit === null ? DEFAULT_EAT_MIN : eatHit
      eatMinSelect.value = String(eatMin)
      // 以「刚应用配置」为新起点，避免刷新后立刻补播一次
      lastEatAt = Date.now()
    }
      ...
      .then(function (d) {
        ...
        if (d.seq > lastCostSeq) {
          ...
          if (d.turn !== null && d.amount !== null) {
            // 主会话的这一轮：顺手播一次「吃饭」动画（气泡照旧弹，一上一下不互相遮挡）。
            // 子代理完成的轮次（main === false）不播。
            if (d.main !== false) playEating(true)
...
// —— 定时「吃饭」动画：把本体临时换成一段视频，完整播一遍后自动切回静态图 ——
// 素材是 VP9 + alpha 的 webm（610×610，与 DSniang1.png 对位），必须用 <video> 播，
// img 播不了视频。任一环不成立（素材缺失 / 环境不支持 VP9 / 解码失败 / play 被拒）
// 都整体静默停用：eatAvailable 保持 false，挂件永远只显示本体，不出现破图或黑块。
var eatEl = document.createElement('video')
eatEl.className = 'dshwv-eat'
// muted + playsInline 必须：否则浏览器自动播放策略会直接拒绝 play()
eatEl.muted = true
eatEl.playsInline = true
eatEl.setAttribute('muted', '')
eatEl.setAttribute('playsinline', '')
eatEl.setAttribute('webkit-playsinline', '')
eatEl.preload = 'auto'
eatEl.draggable = false
// 叠在 img 之上、气泡之下：同为 absolute 且都没设 z-index，按 DOM 顺序 img 在下；
// 气泡 .dshwv-bubble 有 z-index:1，仍然盖在视频之上。
body.appendChild(eatEl)

var eatAvailable = false // 能力探测 + 预热都通过后才为 true
var eatActive = false // 正在播放（含淡出阶段）
var eatFadeTimer = null // 淡出/复位计时器

function eatCanPlay() {
  try {
    // 返回空字符串 = 不支持（Safari 等）→ 直接静默停用
    return !!eatEl.canPlayType && eatEl.canPlayType('video/webm; codecs="vp9"') !== ''
  } catch (err) {
    return false
  }
}
// 预热：整段 fetch 成 blob URL。路由的 Cache-Control 是 no-store，直链会让
// <video> 每次播放都重下 829KB；转 blob 只下一次，之后都走本地内存。
// 显式补 type：部分环境不会从 blob 推断出 webm。
function warmupEating() {
  if (!eatCanPlay()) return
  try {
    fetch(EAT_URL, { cache: 'force-cache' })
      .then(function (r) { if (!r.ok) throw new Error('eat ' + r.status); return r.blob() })
      .then(function (b) {
        var blob = b.type === 'video/webm' ? b : new Blob([b], { type: 'video/webm' })
        eatEl.src = URL.createObjectURL(blob)
        eatAvailable = true
      })
      .catch(function () {})
  } catch (err) {}
...
// 立刻中断并切回本体静态图（点击挂件时走这里）
function abortEating() {
  if (eatFadeTimer) { clearTimeout(eatFadeTimer); eatFadeTimer = null }
  eatActive = false
  // 立刻移除播放态：本体图与视频同时淡入/淡出（交叉溶解），不会露出底层硬边。
  // 这条路径覆盖点击中断 / play 被拒 / 解码失败 / setEatMin 关停。
  body.classList.remove('dshwv-eat-mode')
  try {
    eatEl.style.opacity = '0'
    eatEl.pause()
    eatEl.currentTime = 0
  } catch (err) {}
}
// 播完一遍：先淡出，等过渡结束再 pause + 回到第 0 帧 —— 立刻 pause 会停在最后
// 一帧，淡出过程中漏出残影。
function finishEating() {
  ...
  eatEl.style.opacity = '0'
  if (eatFadeTimer) { clearTimeout(eatFadeTimer); eatFadeTimer = null }
  // 移除播放态要等淡出结束（EAT_FADE_MS = 240ms = transition 时长）：
  // 提前移除会让本体图在视频还半透明时就淡入，两层硬边再次重叠。
  eatFadeTimer = setTimeout(function () {
    eatFadeTimer = null
    body.classList.remove('dshwv-eat-mode')
    try {
      eatEl.pause()
...
// 用 ended 驱动切回，不用固定 setTimeout（视频时长会漂）
eatEl.addEventListener('ended', finishEating)
eatEl.addEventListener('error', function () {
  // 解码/加载失败：整体静默停用 + 复位，不留残影
  eatAvailable = false
  abortEating()
})

// 播放一次。ignoreBubble=true 用于「每轮对话完成」这类要立刻见到动画的场景
// （此时消耗气泡照旧弹出，一上一下互不遮挡）；定时路径传 false，气泡占屏时
// 顺延到下一个间隔。返回 true = 真的开播了。
function playEating(ignoreBubble) {
  if (!eatAvailable || eatActive) return false // 播放中再触发直接跳过，不叠加
  if (eatMin <= 0) return false
  if (drag && drag.active) return false // 拖拽中不插播，避免与位移动画打架
  if (!ignoreBubble && (bubbleShown || costBubbleActive)) return false
  eatActive = true
  lastEatAt = Date.now()
  if (eatFadeTimer) { clearTimeout(eatFadeTimer); eatFadeTimer = null }
  // 进入播放态：本体图淡出、视频淡入（同一帧触发，两层交叉溶解，不会看到叠影）。
  // 退出统一在 abortEating() / finishEating() 里移除，两条路径都必须覆盖。
  body.classList.add('dshwv-eat-mode')
  try {
    eatEl.currentTime = 0
    var p = eatEl.play()
    // 自动播放被拒 / 解码失败：静默回退到静态图
    if (p && typeof p.catch === 'function') p.catch(function () { abortEating() })
    eatEl.style.opacity = '1'
  } catch (err) {
    abortEating()
    return false
  }
...
// 调试台手动触发入口（真机上没有调用方）
window.__dshWhalePlayEating = function () { return playEating(true) }

// 定时检查：页面切到后台时只顺延计时、不欠账（回来重新计时，不补播欠下的次数）
function eatTick() {
  if (document.hidden) {
    lastEatAt = Date.now()
    return
  ...
  if (eatMin <= 0) return
  if (Date.now() - lastEatAt < eatMin * 60000) return
  // 无论是否真的播出去（气泡占屏 / 拖拽中被顺延），都往后推一个间隔，
  // 免得每分钟重试一次
  lastEatAt = Date.now()
  playEating(false)
}
setInterval(eatTick, EAT_CHECK_MS)
})()`
...
function apply(ctx) {
    let imageBytes = null
    const extraImageBytes = new Map()
    let balanceCache = null
    let balanceInFlight = null
    let gifBytes = null
    let eatingVideoBytes = null
    ...
    function finalizeTurn(sessionId) {
      ...
      if (agg && agg.cost > 0) {
        // main:false = 这一轮来自子代理（spawn/fork）。消耗气泡照旧显示，
        // 但前端的定时「吃饭」动画只认主会话的轮次（见 last-turn.json）。
        lastTurn = { turn: agg.turn, amount: agg.cost, tokens: agg.tokens, ts: agg.lastTs, main: !agg.subagent }
        lastTurnSeq++
      }
    ...
    function handleSessionEvent(sessionId, event, isSub) {
      try {
        ...
        if (!agg || agg.turn !== turn) {
          ...
          agg = { turn, cost: 0, tokens: 0, lastTs: Date.now(), subagent: !!isSub }
    ...
    // DSH 的 session 对象结构是 { header: { id, origin } }：origin === 'subagent'
    // 表示该会话由子代理（spawn/fork）派生，判据与官方 desktop 插件一致。
    // 历史代码读的 session.id 在 DSH 上恒为 undefined，会让所有会话落进同一个桶
    // （主会话与子代理互相串账），故统一走 header；header 缺失时回退旧字段。
    function sessionKey(session) {
      const header = session && session.header ? session.header : null
      if (header && header.id !== undefined && header.id !== null) return String(header.id)
    ...
    disposers.push(ctx.on('session/event', (session, event) => {
      handleSessionEvent(sessionKey(session), event, isSubagentSession(session))
    }))
    // 会话销毁时清理残留聚合，避免内存泄漏
    disposers.push(ctx.on('session/disposed', (session) => {
      if (session) turnAggs.delete(sessionKey(session))
    }))
    ...
    // 吃饭动画视频：一次性读入内存缓存（约 829KB）。与 loadGif/loadImage 一致，
    // 换素材后需重启 DSH 才会生效。
    function loadEatingVideo() {
      if (eatingVideoBytes) return eatingVideoBytes
      for (const p of EATING_VIDEO_CANDIDATES) {
        try {
          const bytes = fs.readFileSync(p)
          if (bytes && bytes.length > 0) {
            eatingVideoBytes = bytes
            return bytes
          }
        } catch (err) {}
      }
      throw new Error('eating video not found')
    }
    ...
    function loadExtraImage(slot) {
      const cached = extraImageBytes.get(slot.route)
      if (cached) return cached
      for (const name of slot.names) {
        try {
          const bytes = fs.readFileSync(path.join(PACKAGE_ROOT, 'assets', name))
          if (bytes && bytes.length > 0) {
            extraImageBytes.set(slot.route, bytes)
            return bytes
          }
        } catch (err) {}
      }
      throw new Error('extra whale image not found: ' + slot.route)
    }
        ...
        try {
          ...
          if (parsed && typeof parsed.scale === 'number') {
            return {
              ...
              eatMin: normalizeEatMin(parsed.eatMin),
            }
          }
        } catch (err) {}
      }
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, eatMin) {
      ...
      const em = normalizeEatMin(eatMin)
    ...
    // 额外素材：EXTRA_IMAGES 里每项注册一条固定路由
    for (const slot of EXTRA_IMAGES) {
      disposers.push(ctx.webServer.register({
        kind: 'exact',
        path: '/dsh-whale/' + slot.route,
        handler: (req, res) => {
          try {
            const bytes = loadExtraImage(slot)
            res.writeHead(200, {
              'Content-Type': 'image/png',
              'Cache-Control': 'no-store',
              'Content-Length': String(bytes.length),
            ...
            res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
            res.end('extra whale image unavailable: ' + String((err && err.message) || err))
          }
        },
      }))
    }

    // 吃饭动画（<video> 播放）：Content-Type 必须是 video/webm。
    // 不实现 Range：前端走 Blob 预热（fetch 全量 → object URL），video 不会向本
    // 路由发 Range 请求；素材仅 829KB，整段返回即可（返回 200 全量对 Range 请求
    // 也是合法响应，浏览器会正常播放）。
    disposers.push(ctx.webServer.register({
      kind: 'exact',
      path: '/dsh-whale/eating.webm',
      handler: (req, res) => {
        try {
          const bytes = loadEatingVideo()
          res.writeHead(200, {
          ...
          res.writeHead(404, { 'Content-Type': 'text/plain; charset=utf-8' })
          res.end('eating video unavailable: ' + String((err && err.message) || err))
        }
      },
    }))
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: (req, res) => {
        ...
        // main:false 表示该轮由子代理（spawn/fork）完成 —— 前端据此决定是否
        // 播放「吃饭」动画（子代理不播）。
        const payload = lastTurn
          ? { ok: true, seq: lastTurnSeq, turn: lastTurn.turn, amount: lastTurn.amount, tokens: lastTurn.tokens, ts: lastTurn.ts, main: lastTurn.main !== false }
          : { ok: true, seq: 0, turn: null, amount: null, tokens: null, ts: null, main: true }
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify(payload))
      },
    }))
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.eatMin)
