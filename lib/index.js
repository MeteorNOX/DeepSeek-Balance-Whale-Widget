...
export default {
  ...
  apply(root) {
    ...
    // —— 等待用户交互（issue #160）：当前挂起的「提问 / 授权」——
    // dsh 需要你回答（ask_user_question）或批准（沙箱/工具授权）时，本轮并不会 turn/end，
    // 所以任务结束音不会响；这里单独记下挂起项，前端轮询 wait.json 时据此提示。
    // 只保留最新一条：音效只在挂起出现的那一刻响一次，解除即清空（不区分会话，保持简单）。
    let waitPending = null // { kind: 'question' | 'approval', id, ts }
    function markWaitPending(kind, id) {
      waitPending = { kind, id: String(id), ts: Date.now() }
    }
    function clearWaitPending(kind, id) {
      if (waitPending && waitPending.kind === kind && waitPending.id === String(id)) waitPending = null
    }
    ...
    function handleSessionEvent(sessionId, event) {
      try {
        ...
        // 等待用户交互的两个瞬间：提问挂起 / 授权请求（以及各自的解除）
        if (type === 'tool/call') {
          if (d.name === 'ask_user_question' && d.callId) markWaitPending('question', d.callId)
          return
        }
        if (type === 'tool/result') {
          const block = d.message && Array.isArray(d.message.content) ? d.message.content[0] : null
          if (block && block.toolCallId) clearWaitPending('question', block.toolCallId)
          return
        }
        if (type === 'approval/asked') { if (d.id) markWaitPending('approval', d.id); return }
        if (type === 'approval/decided') { if (d.id) clearWaitPending('approval', d.id); return }
        if (type === 'turn/end') {
    ...
    // 用量相关设置(任务结束音 / 等待用户交互音 / 余额预警 / 今日预算)存于用量账本 settings
    function usageSettingsDefaults() {
      return {
        // 任务结束音：默认关闭，但默认已选中内置的 Minecraft·经验球（exp_orb），打开即用
        taskEnd: { on: false, sel: 'frag:exp_orb' },
        // 等待用户交互音（issue #160）：dsh 需要你回答 / 批准时响一声。
        // 两个状态各自独立选音；默认关闭，但已预选内置音，打开即用。sel 为空 = 静音。
        waitQuestion: { on: false, sel: 'frag:end_a' },
        waitApproval: { on: false, sel: 'frag:exp_orb' },
    ...
    function readUsageSettings() {
      ...
      if (s.waitQuestion && typeof s.waitQuestion === 'object') d.waitQuestion = Object.assign({}, d.waitQuestion, s.waitQuestion)
      if (s.waitApproval && typeof s.waitApproval === 'object') d.waitApproval = Object.assign({}, d.waitApproval, s.waitApproval)
    ...
    function writeUsageSettings(patch) {
      ...
      if (p.waitQuestion && typeof p.waitQuestion === 'object') led.settings.waitQuestion = Object.assign({}, led.settings.waitQuestion || {}, p.waitQuestion)
      if (p.waitApproval && typeof p.waitApproval === 'object') led.settings.waitApproval = Object.assign({}, led.settings.waitApproval || {}, p.waitApproval)
    ...
    disposers.push(registerRoute({
      ...
      handler: (req, res) => {
        // 当前是否有挂起的「提问 / 授权」（issue #160）：前端每秒轮询，出现新 id 时播提示音
        res.writeHead(200, JSON_HEADERS)
        res.end(JSON.stringify({ ok: true, pending: waitPending }))
      },
    }))
