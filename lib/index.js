...
export default {
  ...
  apply(ctx) {
    ...
    function usageRecordsPayload() {
      ...
      return {
        ok: true, version: '0.3.12', today: todayData, days7,
    ...
    function publicBalance(payload) {
      ...
      return {
        ...visible, version: '0.3.12', isPeak: isPeakTime(nowSec),
    ...
    // —— 官方 DSH 桌面端（Electron）适配（issue #152 / #153）——
    // 桌面壳的 index.html 是**从安装包静态 dist 直接读盘**的（`dsh-app://app/`），
    // 永远不经过宿主的 renderIndex()，所以上面的 tapIndex 在桌面端不生效 —— 于是"宿主半区活着、
    // 客户端半区从未被请求"。桌面端走的是**结构化行注入**：宿主启动时 collectIndexInjections()
    // 经 IPC 把行表交给渲染层，渲染层按 kind 逐行应用（见 @deepseek-ai/dsh-host-webserver 的
    // injections.d.ts：`{ kind:'script-src'; placement:'head'|'body'; src:string }`，外部经典脚本）。
    // 这里补同一行；**两条并存**：Web 形态仍走 tapIndex，两边都只会注入一次（各自去重）。
    disposers.push(ctx.on('webserver/index-inject', (table) => {
      try {
        if (!Array.isArray(table)) return
        for (const row of table) {
          if (row && row.kind === 'script-src' && row.src === '/dsh-whale/widget.js') return
        }
        table.push({ kind: 'script-src', placement: 'body', src: '/dsh-whale/widget.js' })
      } catch (err) {}
    }))
