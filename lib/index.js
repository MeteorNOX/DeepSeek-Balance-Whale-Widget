...
displaySelect.title = '选择气泡展示的账户与内容'
displaySelect.addEventListener('change', function () { setDisplayProvider(displaySelect.value) })
...
rowDisplay.appendChild(displaySelect)
...
function displayModeOf(dp) {
  var id = dp || 'deepseek'
  return id.indexOf(':usage') !== -1 ? 'usage' : 'balance'
}
...
function computeDisplayFigure(acc, mode) {
  ...
  // 已用配额态 → 展示已用（token 级，不受用户余额影响）
  if (mode === 'usage') return acc.used
  // 余额态：
...
function newapiBalanceHint(acc) {
  if (!acc.userTokenConfigured) return '未配置访问令牌 · 设置'
...
function relayHint(acc, mode) {
  ...
  if (acc.used === undefined || acc.used === null) {
    var w2 = acc.windows && acc.windows.length ? acc.windows[0] : null
    if (w2 && w2.used !== undefined && w2.used !== null) return '已用 ' + fmt(w2.used, acc.currency)
    return '已用 --'
  }
  return '已用 ' + fmt(acc.used, acc.currency)
}
function buildDisplaySelect() {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    var label = p.label || p.accountId
    var bOpt = document.createElement('option')
    bOpt.value = p.accountId
    bOpt.textContent = label + ' · 余额'
    displaySelect.appendChild(bOpt)
    var uOpt = document.createElement('option')
    uOpt.value = p.accountId + ':usage'
    uOpt.textContent = label + ' · 已用配额'
    displaySelect.appendChild(uOpt)
  }
  displaySelect.value = state.displayProvider || ''
}
function setDisplayProvider(v) {
  state.displayProvider = v || 'deepseek'
  displaySelect.value = state.displayProvider
  // 切换即时生效：accounts 已在 payload 里，直接重置滚动数再渲染
  shown = null
  buildCurrencySelect()
  saveConfig()
  render()
  ...
  fetch(BALANCE_URL, { cache: 'no-store', signal: ctrl ? ctrl.signal : undefined })
    ...
    .then(function (data) {
      if (data && data.ok) {
        ...
        if (!state.displayProvider && data.displayProvider) state.displayProvider = data.displayProvider
        ...
        if (data && data.displayProvider && !state.displayProvider) state.displayProvider = data.displayProvider
...
fetch(SIZE_URL, { cache: 'no-store' })
  ...
  .then(function (d) {
    ...
    if (d && typeof d.displayProvider === 'string' && d.displayProvider !== '') {
      state.displayProvider = d.displayProvider
    }
...
function apply(ctx) {
    ...
    function relayFailTtl(r) {
      // 本地 gate 拒绝：等本窗口滚动即可
      if (r.error === 'rate-limited') return RELAY_RATE_WINDOW_MS
      // 站点 429：按它的 retry-after 封禁时长做负缓存（实测约 20 分钟），
      // 拿不到 header 时退回 5 分钟
      if (r.error === 'http-429') {
        if (typeof r.retryAfterMs === 'number' && r.retryAfterMs > 0) {
          return Math.min(r.retryAfterMs, 24 * 3600 * 1000)
        }
        return RELAY_RATE_WINDOW_MS
      }
    ...
    function normalizeProviders(cfg) {
      ...
      for (const raw of list) {
        ...
        out.push({
          ...
          // New API 用户余额（/api/user/self）：访问令牌 + 用户 ID 的凭据引用名。
          // 令牌在站点 个人设置 → 安全设置 生成；用户 ID 在个人设置页顶部。
          // 多账户同站点需要不同 ID 时，可给每个 provider 单独指定凭据名。
          ...(type === 'newapi'
            ? {
                userToken: typeof raw.userToken === 'string' && raw.userToken.trim() !== ''
                  ? raw.userToken
                  : DEFAULT_USER_TOKEN_CREDENTIALS.userToken,
                userId: typeof raw.userId === 'string' && raw.userId.trim() !== ''
                  ? raw.userId
                  : DEFAULT_USER_TOKEN_CREDENTIALS.userId,
              }
            : {}),
        })
      }
    ...
    // 解析 displayProvider：'{accountId}' = 余额态，'{accountId}:usage' = 已用配额态。
    function resolveDisplay(displayProvider, accounts) {
      ...
      if (colon !== -1) {
        ...
        mode = dp.slice(colon + 1) === 'usage' ? 'usage' : 'balance'
      }
    ...
    async function getBalancePayload() {
      ...
      const displayProvider = display.accountId + (display.mode === 'usage' ? ':usage' : '')
      ...
      if (current && current.type === 'deepseek') {
        ...
        // 中转站：顶层字段别名到当前显示账户（余额态→balance；已用态→used）。
        const picked = pickDisplayFigure(current, display.mode, displayCurrency)
        const figure = display.mode === 'usage'
          ? current.used
          : picked && picked.value !== undefined ? picked.value : current.balance
        full = {
          ok: current.ok,
          totalBalance: figure !== undefined ? figure : null,
          currency: display.mode === 'usage' ? current.currency : (picked ? picked.currency : current.currency),
    ...
    function writeSizeConfig(scale, sound, vol, soundSet, usageMode, peakMode, bubbleOn, turnCostOn, turnCostCloseMs, scrollGapOn, scrollGapPx, displayProvider, displayCurrency) {
      ...
      handler: async (req, res) => {
        if (req.method === 'PUT' || req.method === 'POST') {
          try {
            ...
            const result = writeSizeConfig(scale, parsed.sound !== false, parsed.vol, parsed.soundSet, parsed.usageMode, parsed.peakMode, parsed.bubbleOn, parsed.turnCostOn, parsed.turnCostCloseMs, parsed.scrollGapOn, parsed.scrollGapPx, parsed.displayProvider, parsed.displayCurrency)
    ...
    disposers.push(ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          await ctx.credentials.set(p.userId || DEFAULT_USER_TOKEN_CREDENTIALS.userId, userId)
          await ctx.credentials.set(p.userToken || DEFAULT_USER_TOKEN_CREDENTIALS.userToken, token)
