...
//   ① 栅栏不变量    —— 新增路由绕过 requestRejection（凭据外带 / 内网可达，安全修复 S1；issue #136 fail-open）
//   ② 路由对等      —— 前端 fetch 了宿主没注册的路径（两个巨石文件各改一半 → 点了没反应、静默 404）
//   ③ 发布产物自洽  —— 装出来的包里缺 assets/ 或 dsh.bundle.patch 指错文件（插件整包起不来）
//   ④ 门禁一致性    —— 只改了 ci.yml 忘了 publish.yml（发布路径上某个检查被悄悄摘掉）
//   ⑤ 注入面变更    —— innerHTML = <非常量>（issue #143；新增未转义入口要有人看见）
//
// 设计原则：
...
// ---------------------------------------------------------------------------
run('1', '路由栅栏不变量（所有 /dsh-whale/* 路由都必须过 requestRejection）', (problems, notes) => {
...
// ---------------------------------------------------------------------------
run('2', '路由对等（前端 fetch 的路径必须已被宿主注册）', (problems, notes) => {
...
// ---------------------------------------------------------------------------
run('3', '发布产物自洽（npm pack 出来的包里，插件必须能起来）', (problems, notes) => {
...
// ---------------------------------------------------------------------------
run('4', '门禁一致性（ci.yml 与 publish.yml 不许各查一半）', (problems, notes) => {
  const REQUIRED = [
...
// ---------------------------------------------------------------------------
run('5', '注入面变更（innerHTML = <非常量> 必须被显式看见）', (problems, notes) => {
