...
- 🎚️ **主菜单**：大小滑块、音效开关/音量、峰值文案、泡泡开关、每轮消耗开关 + 「自定义提示」（每轮消耗内容 / 自动关闭秒数 / 任务结束音效 / 等待提问·授权音效）、吸附设置、角色/资源管理、自定义泡泡、余额预警、今日预算
...
- ⏳ **等待你操作音**（issue #160）：dsh **需要你回答提问**、或**需要你批准授权**时各响一声。这两种时刻本轮还没结束，所以任务结束音不会响 —— 现在它们各自可以在菜单 → 每轮消耗提示 → 「自定义提示」里选音（与任务结束音同一套音效库，可选「静音」）。默认关闭，打开即用内置音
...
| `.dshw-usage.json` | 记账账本 + 按日余额观测/校正摘要 + 用量设置（任务结束音、等待提问/授权音、余额预警、今日预算、每轮消耗提示内容） |
...
- `/dsh-whale/wait.json` → 200 JSON，含 `{ok:true, pending}`；`pending` 为当前挂起的「提问 / 授权」（`{kind:'question'|'approval', id, ts}`）或 `null`（issue #160 的等待提示音数据源）
- 浏览器 F5 后右下角出现挂件
...
- **需要我回答/批准时没响**：这两个音效也默认**关闭**（默认分别预选内置的 **A** 与 **Minecraft·经验球**）；到菜单 → 每轮消耗提示 → 「自定义提示」里打开「提问等待音效」/「授权等待音效」并点「保存」即可。选成「静音」则不播；同一个提问在刷新页面后不会重复响。
