...
- 📋 **侧边栏每会话消耗显示**：Web 左侧会话列表的每个会话行「标题」与时间之间显示该会话累计消耗金额（浅边框小胶囊，如 `¥0.82`）；菜单里可开关「会话消耗」
  - **悬停明细浮层**：鼠标移到胶囊上弹出明细（自绘浮层、跟随行定位、滚轮/滚动自动收起）——合计金额、「本会话」与「子代理 N 个」两段各自的金额 / web 检索轮次与费用 / 缓存·输入·输出 token / 模型，并逐个列出每个子代理的标题与花费（含它自己的孙代理个数）
  - **模型感知计价**：按日志里**每个事件真实模型**（flash / pro 分档）+ **事件时刻的峰谷**精确换算，与「每轮消耗」完全同一口径；混合模型会话按各自模型分别计价（明细里显示全部模型）
  - 进行中的会话直接读内存事件表实时计算（每轮结束自动更新：轮询 8 秒 + 新轮次即时刷新；浮层开着时数据刷新会就地重绘）；历史会话在 `dsh web` 启动时**后台预热扫描**，打开页面即有大部分金额（其余几秒内补齐，先显示已就绪的）
  - **子代理费用自动聚合**：胶囊金额 = 会话自身 + 全部子代理（spawn/fork 子会话及其孙辈）消耗；明细里拆成「本会话 / 子代理 N 个」两段，并列出每个子代理各自花了多少
  - **web 检索费用也计入**：`web_search` 每轮都会用本账号真实调用一次模型，但 DSH 不会把这次调用的 token 写进会话事件 —— 早期版本因此漏掉长任务里最大的一笔（实测某会话 775 轮检索 ≈ ¥21 完全没显示）。现在按轮次计入，明细里显示「web 检索 N 轮 ¥X」；单价默认 ¥0.027/轮（由账号账本反推校准），可在 `.dshw-size.json` 里用 `searchUnitCost` 覆盖
  - 无需任何凭据，无需平台令牌
...
- **悬停明细浮层**：鼠标移到胶囊上弹出明细面板（自绘 fixed 浮层，定位在胶囊右侧、超出屏幕自动翻到左侧；滚动/滚轮/窗口缩放自动收起；面板 `pointer-events:none` 不抢悬停）
  - 顶部：合计金额 + 子代理个数
  - **本会话**：金额、web 检索轮次与费用、模型、缓存/输入/输出 token
  - **子代理 N 个**：整段金额、web 检索轮次与费用、token 合计，并**逐个列出每个子代理**（标题取官方 `session.list`，过长截断）+ 各自花费 + 该子代理自己的检索轮次/孙代理个数；超过 8 个折叠为「还有 N 个」
  - 底部：口径说明（有 web 检索时标明按 ¥X/轮 估算，否则标明按官方 token 定价 × 峰谷换算）
  - 面板打开期间数据刷新（每轮结束/轮询）会就地重绘，不打断悬停
- **子代理聚合**：徽标金额 = 主子会话 + 其全部子代理（`origin=subagent` 的子会话，含孙辈）消耗之和；明细里拆成「本会话 / 子代理 N 个」两段；子代理自身的 `web 查询`/工具等消耗都计入各自子会话，再并入父会话胶囊
- **web 检索（不落 token 账单的调用）也计入**：DSH 的 `web_search` 工具**每一轮**都会用本账号向 Anthropic 兼容端点发一次 `deepseek-v4-flash` 请求（会话事件 `web/deepseek-search-llm-request`），但这次调用的 token 用量**不会**写进 `assistant/message`，只数 usage 的账会漏掉大头。插件按**轮次**计价并计入胶囊与子代理聚合：
  ...
  - 明细面板里显示「web 检索 N 轮 ¥X」；想改单价就在 `$DSH_HOME/.dshw-size.json` 里加 `searchUnitCost`（元/轮，改完重启 `dsh web`）
...
- `/dsh-whale/session-costs.json` → 200，含 `{"ok":true,"searchUnit":0.027,"costs":{"<sessionId>":{"amount":...,"own":...,"sub":...,"subCount":...,"usage":...,"aux":...,"search":...,"ownDetail":{...},"subDetail":{...},"children":[{"id":...,"amount":...}],"models":{...}}}}`（`amount = usage + aux`；`search` 为 web 检索轮次；`ownDetail/subDetail` 是「本会话 / 子代理」两段明细，`children` 是每个直接子代理的整组花费，供悬停面板展示；首次请求后历史会话在后台扫描，几轮轮询内补齐）
- 浏览器 F5 后右下角出现挂件
