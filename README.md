...
#### 为什么需要它：第二把 DeepSeek key 原本无处安放

在这之前，想加第二把 DeepSeek key 只有两条路，**两条都不通**：

1. **把它填进内置项的凭据名** → 直接覆盖掉自己那把，两个账号只能二选一，不能同时看
2. **用「+ 添加模型（自定义 API）」新建一条** → DeepSeek 官方模板的凭据名固定是 `DEEPSEEK_API_KEY`，
   填第二把 key 只会**又覆盖掉第一把**；就算把凭据名改成别的，那也只是**多出来一张卡片** ——
   它没有内置项的「已观测消费 / 余额校正 / 今日已用余额差记账 / 泡泡里的 DeepSeek 余额」这些能力，
   等于"查得到数额，但用不了这套记账"

所以实际效果就是：**在「添加模型」里加进来的第二把 DeepSeek key，查不到余额（或者只能覆盖第一把）**。

v0.3.3 起把「账号」提成一等公民，直接在**内置 DeepSeek 自己的设置里**加账号：每个账号一把 key
+ 一个名字，**同时保留内置项的全部能力**（今日已用、已观测消费、余额校正、泡泡余额），
而且**各账号分开记账**、互不污染。
...
挂件的**界面铬件**（菜单文字、按钮、选中态、说明圈）配色可以换，也可以自动跟着你换的角色走：

- 🎨 **入口**：右键小鲸鱼 → 菜单「角色」那一行的 **「配色」** 按钮
- 🐳 **跟随角色主色（默认）**：从当前角色图里提取主色，自动生成整套配色；**换角色就跟着变**
- 🎯 **自选颜色**：取色器直接选，或者点「快捷色」——那是围绕角色主色旋转色相得到的 6 个同族色，方便"像角色但换个调子"
- 🔵 **内置蓝（默认）**：回到原来的 `#203170`，一键切换
- 🧮 **配色怎么算的**：主色只决定**色相**，界面色按固定亮度生成（选中态 0.30 / hover 0.42），所以浅色泡泡上的文字始终清楚；按钮上的文字颜色按 **WCAG 对比度（≥4.5）** 自动取黑或白。取不到主色时（例如 GIF 角色）回落内置蓝，不会把界面配成一片糊
- 📦 **存哪**：配色存在 `whale-roles/roles.json` 的 `appearance` 段（`{ mode, color, roleId }`），和角色同一份文件，换角色天然一起读；不写任何密钥
- 🧩 **不影响泡泡**：配色只作用于界面铬件。泡泡里每个模块的文字颜色/底色仍由泡泡编辑器自己管，所以换配色**不会**打乱你已经调好的泡泡排版

> 实现说明：挂件自身的 CSS 已全部改成 `var(--dshw-ui, <原色>)` 形式，不设变量时外观与旧版**完全一致**（可随时用「内置蓝」回到原点）。角色主色提取优先用安装环境里的 `sharp`（按绝对路径加载，因为 link: 安装时解析不到裸模块名），取不到时用随包的最小 PNG 解码器兜底（`lib/png.mjs`，8bit 非隔行 PNG），因此「跟随角色主色」在任何安装形态下都真的会工作。
