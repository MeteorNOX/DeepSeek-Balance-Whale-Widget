...
- 🥣 **可选挂饰**：菜单 → 挂饰 →「铁盆（头顶饭盆）」；选择「无」可取下，默认不佩戴。佩戴时隐藏头顶呆毛，取下后恢复，原图文件保持不变。选择保存在现有 `.dshw-size.json` 中，刷新后恢复；铁盆随本体缩放、镜像、拖拽和按压变形，透明区域允许点击下层页面。
...
│   ├── iron-basin.png    # 透明底铁盆挂饰（深蓝描边、银灰配色）
│   ├── rua.gif           # 随机台词 gif（可选）
